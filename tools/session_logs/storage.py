...
def store_artifact(
  ...
  *,
  ...
  if not transcript_exists and not provenance_exists:
  ...
  if not transcript_exists or not provenance_exists:
    return StorageResult(
      action="preserved",
      transcript_path=transcript_path,
      provenance_path=provenance_path,
  ...
  if update.action == "unchanged":
    action = "unchanged" if existing_text == artifact.text else "preserved"
