...
恒久的な合格値や削減目標にしない。要求ID、文書path、内容識別値、版、件数、file集合、呼出し先、設定、
形式定義への直接参照を機械的に列挙し、固定した確認基準に含まれない資料への参照、または確認基準の
変更・廃止に関係する候補だけを人が確認する。全試験を一件ずつ要求へ対応付けず、矛盾を実証できない
試験は現状維持とする。
...
記録した、試験整理自体を目的化して全件の詳細確認へ進みかけた方針訂正である。
