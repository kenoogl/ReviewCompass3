...
することを機械確認する。commit識別値は呼出し側から受け取らずGitから取得する。
...
確認では、作業ごとに必要な働き、入出力、失敗時の扱い、安全条件、既知の参考処理、必要・禁止の
構文副作用を先に確認する。機械処理は、参考処理と直接関係する処理を探した後、同じProfileと
Comparison Discoveryを使って、別の機能名・file名にある共通内部部品を現在の全コード集合から探す。
必要な働きの自然文を機械が処理名へ推測変換せず、Humanが確認した作業別計画schema 2を入力にする。
作業ごとの参考path・処理は時点入力であり、将来の検索元または中央一覧にしない。参考処理がない働きも、
処理名の手掛かりまたは構文副作用から全コードを確認し、候補がなければ未対応として人へ返す。

検索結果は、必要な働きごとに候補、候補となった根拠、禁止副作用との衝突、sourceに明記された
`stable`・`provisional`・`stopped`または未宣言を観測として示す。この表示だけから採用・停止を
自動決定せず、Decisionとの意味照合をHumanへ残す。旧schema 1のpath中心検索は過去証拠の再読込みに
残すが、新しい実装前確認の通常入力には必要な働き中心のschema 2を使う。

正式採用済み、暫定・保留、使用停止、履歴だけという区分は、
...
現在の通常経路では、上記一操作入口がWork 4Aの処理一覧と比較候補、およびWork 4Bの再利用検索記録と
鮮度確認を既存の機械処理として呼ぶ。作業別計画は時点証拠であり、将来の検索元または中央一覧にしない。
検索結果が現在のコードへ結び付かない、検索範囲が空で変更検知ができない、検索元の方針参照が古い、
または証明書出力先が既に存在する場合は、実装開始の合格根拠にしない。対象範囲を現在状態から再生成し、
再現可能な検索結果と利用判断を得るまで製品コードへ着手しない。入口の完了結果にあるライフサイクルと
再利用方法の裁定待ち表示は、人の判断を代行しない。
...
`records/development/2026-08-15-committed-source-formal-search-precheck-decision-v1.md`、
`records/development/2026-08-15-formal-code-reuse-search-one-operation-entry-implementation-plan-v1.md`へ固定する。
