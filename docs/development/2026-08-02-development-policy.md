...
現在の実コードから既存処理を機械検索する。過去に作った処理一覧を現在の一覧として流用せず、対象と
なるコード範囲の変更、追加、欠落がないことを鮮度確認する。

確認では、対象pathと予定する責務名から候補を列挙し、正式採用済み、暫定・保留、履歴だけの処理を
混ぜない。各候補について、再利用、拡張、統合、または理由を付けた分離のどれにするかを、既存の
...
条件へ接続されていなかった事象と、利用者の即時修正指示である。実測と判断は
`records/development/2026-08-15-safe-storage-preimplementation-code-management-routing-evidence-v1.md`へ固定する。
