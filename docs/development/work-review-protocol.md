...
`docs/development/legacy-pilot-review-collaboration.md`の規則を使う。modeの明示が
...
- [ ] blockingにした各Findingが§11の4類型のどれに該当するかを明記した。
- [ ] 次のHuman判断または作業を一つだけ示した。

## 11. レビューの比例原則

レビューの深さは対象の段階とriskで決める。指摘の量・網羅性・文書量そのものを品質と
しない（§8と同旨）。本節は、レビューが本質を越えて際限なく細部へ拡大することを防ぐ
歯止めであり、§4の順序と§5のoracleを変えない。

### 11.1 blockingにできる根拠（閉じた列挙）

Findingには`blocking`／`non-blocking`の別と、確認する段階（scope／implementation／
completion）を必ず付ける。blockingにできる根拠は次の4類型だけである。

1. 上流authority（Plan、承認Decision、Requirement）との矛盾
2. Human境界・必要な承認の欠落
3. 「誤った合格」を実証できる受入条件・検証の欠陥（偽陰性・偽陽性）。原則として
   機械実行できる反証を付ける
...
| レビュー段階 | blocking判定の対象 | non-blockingへ回すもの |
| --- | --- | --- |
| 範囲レビュー | 境界の妥当性：上流整合、範囲、Human境界、risk、受入条件の方向 | 実装手段の詳細（command option、区切り文字、閾値、fixture構成）→「実装時確認事項」として完了レビューで検証 |
| 完了レビュー | 受入条件の成立、報告と事後状態の一致、禁止操作、再現可能性 | scope外の将来設計・改善提案→改善候補またはdefer記録 |

### 11.3 類型一括掃討

偽陰性・偽陽性の欠陥類型をblockingで1件見つけたreviewerは、同じ類型の変種（対象の
種別、境界値、表現形）を同じレビュー周回で掃き出し、反証を複数周回へ小出しにしない。
周回の追加はHumanの受け渡しと全Test再実行を伴うためである。

### 11.4 scope外への逃がし先

指摘が、scope宣言のfixture・対象に存在しないもの（例：宣言外のSCM object種別）へ
拡大する場合は、blockingにせず「後続slice候補」としてdefer記録する。判定基準は
scopeの範囲宣言である。scope自体の欠陥（類型1・4）とはこの逃がし先を区別する。

### 11.5 軽量な再評価

判定の過剰または過小が判明した場合、旧レビューrecordを書き換えず、短い再評価record
を追加して判定とFindingの分類を訂正する。再評価recordも単独commitし、Humanへ報告
する。過剰判定の訂正コストを低く保つことで、「迷ったら全部blocking」へ倒れる誘因を
減らす。
