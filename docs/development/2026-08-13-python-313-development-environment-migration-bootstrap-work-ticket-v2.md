# Python 3.13開発環境移行 軽量作業票 v2

- 作業票ID：`BTW-PYTHON-313-DEVELOPMENT-ENVIRONMENT-MIGRATION-001`
- 作成日：2026-08-13
- 状態：`awaiting_correction_review`
- 基準コミット：`76ec335fe69948e982141307244d45b0ab10d9f6`
- 危険度：`high`
- 先行版：`docs/development/2026-08-13-python-313-development-environment-migration-bootstrap-work-ticket-v1.md`
- 先行版開始前レビュー：`records/development/2026-08-13-python-313-development-environment-migration-bootstrap-start-review-v1.md`
- 作業担当：開始承認後の主担当
...
| 先行版開始前レビュー | `96ad99ff4da9b713b321870ce47f267f4a57c016e69963582ff04f76f806c04a` | Homebrew副作用抑止不足1件を修正する根拠 |
...
Homebrew 6.0.14は`/opt/homebrew/bin/brew`にあるが、Python 3.13は未導入である。ローカルの公式ヘルプは、
install前の自動更新、古い依存先の検査・更新または再導入、install後のcleanup、install済み対象のupgradeが
既定で起き得ると示す。現在の依存木は、直接依存4件に加えて`ca-certificates`と`readline`を含む。
...
- 実行直前にHomebrewが示す`python@3.13`の依存閉包以外の導入・更新、既存Homebrew packageの削除。
...
利用者が、外部取得と`/opt/homebrew`への書込みを明示承認した後だけ行う。まず同じ抑止条件で、実行直前の
依存閉包と導入計画を取得する。
...
env HOMEBREW_NO_AUTO_UPDATE=1 \
  ...
  /opt/homebrew/bin/brew deps --tree python@3.13
env HOMEBREW_NO_AUTO_UPDATE=1 \
  HOMEBREW_NO_INSTALLED_DEPENDENTS_CHECK=1 \
  HOMEBREW_NO_INSTALL_CLEANUP=1 \
  HOMEBREW_NO_INSTALL_UPGRADE=1 \
  /opt/homebrew/bin/brew install --dry-run python@3.13
```

許可する対象は、`python@3.13`と最初のcommandが示す依存閉包だけである。2026-08-13の実測では、直接依存の
`mpdecimal`、`openssl@3`、`sqlite`、`xz`と、推移的依存の`ca-certificates`、`readline`を含む。実行時の
依存閉包を正とし、dry-runがその外を示す場合は停止する。

本実行は同じ4変数を設定し、TTY上で導入計画と確認入力を表示する。
...
env HOMEBREW_NO_AUTO_UPDATE=1 \
  ...
  HOMEBREW_NO_INSTALL_CLEANUP=1 \
  HOMEBREW_NO_INSTALL_UPGRADE=1 \
  /opt/homebrew/bin/brew install --ask python@3.13
```

TTYの導入計画が直前のdry-runと一致し、依存閉包内である場合だけ確認へ同意する。計画が違う、閉包外の対象を
含む、または確認入力なしで進もうとする場合は同意せず停止する。4変数で自動更新、既存依存先検査、cleanup、
install済み対象のupgradeを抑止する。
...
- Homebrewの依存閉包、dry-run、TTYの導入計画を同じ4変数の抑止条件で照合し、閉包外の変更前に止まれるか。
...
- Homebrewのdry-runまたはTTYの導入計画が依存閉包外を含む、両者の対象集合が一致しない、または確認入力なしで進む。
...
- Homebrewの依存閉包とdry-runが保存され、本実行計画と一致する。Python 3.13と隔離環境が確認され、
  新しい依存固定が実導入結果と一致する。
