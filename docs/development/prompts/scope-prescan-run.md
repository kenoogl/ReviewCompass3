...
1. 実測は原則**測定ブロック**で行う：実行するコマンド列を宣言JSONに書き、

   ```text
   .venv/bin/python3 -m tools.development.measurement_block \
     --commands <宣言JSONのpath> --output <生成先のpath>
   ```

   で機械生成fileへ固定し、recordは生成物を**参照**して意味の説明だけを書く（数値の転記という
   行為を残さない。2026-08-18新設・fence耐性と切り詰め印つき）。
