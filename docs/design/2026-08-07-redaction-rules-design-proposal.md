...
## 5. 二次の網（高entropy検出）の位置づけ

第2束の反証で6件の抜けが確認された（長さ・複雑さの閾値直下、区切り文字による分断、
広いallow patternによる無効化）。閾値を下げれば誤検出が増え、上げれば漏れが増える。
**この網は本質的に網羅しない。**

したがって層2（機械が支援する。安全保証ではなく誤記検出）と位置づけ、
`tools/development/verification_boundary.py`の宣言へ次を加えることを提案する。

- 対象：`redact_text_strict`の高entropy網
- 区分：`typo_detection_not_safety`
- 担い手：Human（外部送信の判断は網の合格を根拠にしない）

**そのうえで、実際の防御は出口に置く**——派生物を外部へ出す経路自体をHuman承認事項とする。
これは層3の考え方（機械が保証しない部分は明示して人が担う）と同じ構造である。
**出口の承認設計は本提案の範囲外**とし、外部APIレビューのTask Contractで扱う。
...
4. 二次の網を層2と位置づけ、宣言へ加えること
5. 実装順（本提案の承認後、確立済み関門を通して実装）
