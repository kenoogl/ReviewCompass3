...
- 全景と進捗の俯瞰は`docs/current/reviewcompass3-overview-current.md`（全体見取り図。作業単位の
  受入完了時に現在位置入口の更新と同時に状態だけ更新する）を使う。初期開発checklist
...
- セッションログの記録（3系統の全件保全）は`docs/development/prompts/session-log-record-run.md`を入口とする。
...
- 実装開始の根拠に使う正式な再利用検索は`.venv/bin/python3 -m tools.development.formal_code_reuse_search`
  を一操作入口とし（`scope-prescan-run.md`手順5・開発方針123行）、生成した証明書recordのpathと
  SHA-256を契約候補の「権威、証拠」節へ載せる。`start_allowed`がtrueでない結果を合格根拠にしない。
