...
- Pilot期間中、完了した作業単位を未コミットのまま次の作業単位へ進めない。
- 作業単位の完了時と、利用者から「次へ」相当の指示を受けた時は、
  `python3 tools/development/work_unit_transition.py --work-status completed`を実行する。
- preflightが`completed_work_unit_uncommitted`を返した場合は、利用者へコミットをリマインドし、
  コミットされるまで次作業を開始しない。作業中のdirty差分だけではこの状態に分類しない。
- Pilotでは自動コミットを行わず、コミットには利用者の明示指示を必要とする。
