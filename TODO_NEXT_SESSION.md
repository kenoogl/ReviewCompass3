...
- 現在作業：Work 6A中核negative pathの承認範囲をGREENにし、非authority入力の拒否対象を上位文書が名指しする4件へ広げた。checklistのCL-6A-08はHumanが完了を承認した。Work 6Aの段完了ではなく、他10項目は未完了である。
- Task Contract：`none`
...
Humanが恒久検査器をIssueレーンへ載せる形式を判断する。

開始条件：

- CL-6A-08完了反映を含むcommitがcleanであること
- 形式が確定するまで改善候補recordを作成しないこと

完了条件：

- 改善候補recordの形式が確定すること
- 候補recordがvalidatorに合格し、Human triage判断へ渡せること

後続作業：候補からHuman triage判断を経てIssueへ昇格させる。昇格はHumanが判断する。
...
- Human判断待ち：恒久検査器の改善候補recordの形式。過去TODO由来でない候補は既存2形式のどちらにもそのまま載らない。

## stale・deferred

- stale：checklist CL-6A-08は未完了表示から完了へ変わった。RED Evidence§8-2の誤りとGREEN Evidence v1の陳腐化は訂正recordで置換済みである。
