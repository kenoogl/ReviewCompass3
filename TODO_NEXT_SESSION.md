...
- 全体：立て直し計画v5を採用し、開発・レビュー入口をv5へ接続した。第1段『現在位置と現役経路の確定』の軽量作業票v1をコミット済み。
- 現在作業：第1段作業票はawaiting_start_confirmation。対象期間、資料の母集合、分類語彙、対象外、成果物、完了条件のHuman開始確認待ち。現在位置・利用経路表の作成は未開始。
- Task Contract：`none（ブートストラップ立て直し中のため未導入）`

## 現在作業に影響する改善候補／Issue

- `ISSUE-UNREVIEWED-WORK-REVIEW-BACKLOG-001`：`registered`、影響：未レビューの守り役codeを本線へ戻すと誤合格の危険がある、次：第1段の現在位置・利用経路表で現役、使用停止、削除候補、記録のみへ分類する
...
利用者が第1段軽量作業票v1の対象期間、資料の母集合、分類語彙、対象外、成果物、完了条件を確認し、開始可否を判断する。

開始条件：

- 立て直し計画v5と採用判断の対象SHA-256が一致していること
- 第1段軽量作業票v1がcommit 3c0dce6357517148a790af8211aa6f723ccef4a1に固定されていること
- 現在位置・利用経路表、コード変更、外部送信が未開始であること

完了条件：

- 対象期間、資料の母集合、分類語彙、対象外、成果物、完了条件に一回のHuman判断が対応すること
- 不承認または保留なら現在位置・利用経路表の作成を開始しないこと
- 承認時も第1段の作業開始だけを認め、段完了や第2段を承認しないこと

後続作業：開始承認後、作業票の範囲だけで現在位置・利用経路表一件を作成し、別の実行単位による独立完了レビューを一回行う。

## blocker・Human判断待ち

- blocker：技術的blockerなし。作業票の開始条件としてHuman確認を待つ。
- Human判断待ち：第1段軽量作業票v1の開始を承認するか。
...
- 直近の関連Test：文書だけの変更のため新規testなし。立て直し計画v5、採用判断、第1段作業票のpathとSHA-256を機械照合済み。
