...
- 現在作業：作業契約候補v2の変更点確認は修正要だった。複製reviewの問題は解消済みで、残った入力pathとの衝突、配列順、指摘署名、絶対path規則だけを候補v3へ限定訂正した。製品コードと試験は未変更である。
- Task Contract：`TC-RC3-PRODUCT-ONE-ITEM-REVIEW-003 / candidate_v3_pending_limited_independent_review_and_human_approval`
...
固定した作業契約候補v3へ、同じ独立担当が残る2原因の変更点だけを一回確認する。位置指定引数と内容検査の分離、全配列順、指摘署名、絶対pathの4 patternを反証する。

開始条件：

- 作業契約候補v3、v2変更点確認、本TODOが意味単位commitへ固定され、作業場所に未記録差分がない
- レビュー担当はv2とv3のpath・SHA-256、残る2原因、対象commitを受け取り、成果を変更しない
- 製品コード、試験、実利用者資料、外部送信、外部処理を変更または実行しない

完了条件：

- 開始可または修正要の一方を、残る2原因ごとの解消判定と反証結果を伴って返す
- 位置指定引数、復号済み内容、全配列順、指摘署名、停止例・非停止例を照合する
...
- deferred：候補v3の変更点確認と利用者承認までは、製品コード、試験、正式入口、実行名、既存G02、保存、外部送信、外部処理、実利用者資料を変更・実行しない
