...
**伏字化規則の設定登録と保全経路への接続**（候補2。Work 1B後続・機密実施順序2番目。実装済み規則を設定へ登録し、保全経路から呼ばれて実際にマスクが適用されることをTDDで固定する）。

開始条件：

- Humanの着手指示（候補一覧2026-08-08から推奨順の先頭として提示。行き先変更も可）

完了条件：

- 登録GREENと、保全経路で伏字化が実際に適用されるEvidenceの固定

後続作業：**幹線復帰（Work 7A：`local_integrated` deployment E2E）を推奨**。足場課題（参照Digest恒久検査器〔実証4回〕・TODO検証単一入口〔実害3回〕・守り役後追いレビュー）はWork 7Aの区切りごとの足場整備枠で拾う案。

## blocker・Human判断待ち

- blocker：なし。登録済み課題の着手、V1凍結レーンの解除、テストの一斉整理、Work 8前倒しは行わない。Codex側の指示書にある作業はHumanの指示により扱わない。
- Human判断待ち：次の一作業（伏字化規則の登録）の着手指示。行き先の変更指定も可。
