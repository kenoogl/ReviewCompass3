...
- 全体：立て直し計画v5第1〜5段・正式ツール化（契約008〜016）・デプロイ方針決定・評価データ取得（RQ1装置・RQ2 paired trial・論文データ一式・従軸＝運用集計v1〜v7）・測定ブロックと計画／台帳laneの機械化・issue突合まで完了（**詳細は見取り図とgit履歴が正本**。従軸の執筆用固定は台帳追補v1／v2）。論文執筆が進行中：**WSSE 2026 Special Session（締切2026-08-30・LaTeX 5頁・double-blind）＋arXivフルの二本立て**。全景は`docs/current/reviewcompass3-overview-current.md`
- 現在作業：執筆スレッド＝**論文執筆（WSSE短縮版が締切物）**。英語初稿v1＋文献15件BibTeX＋図1 TikZ源＋§5関連研究3本追補まで固定済み（2026-08-20。表現・非主張等の確定事項は草稿v3付記・v4が正本）。**残作業＝WSSE様式LaTeXへの組版と数値の機械照合**。開発スレッド＝**契約016（モデル選択＋記載照合＋登録定型化）まで製品受入完了**（2026-08-20。契約015・016を同日受入＝3 backend×モデル選択体制・terra初起動実証。次の開発作業は順序選択待ち）。執筆体制＝執筆スレッドは`docs/paper/`のみ書く・開発スレッドが正本管理（計画v2 §3の分担規則）
- Task Contract：`なし（契約016受入完了＝2026-08-20の製品受入record。次契約は未定義）`
...
- [全体見取り図（人向け全景。受入完了時にTODOと同時更新）](docs/current/reviewcompass3-overview-current.md) — SHA-256 `5fed8a2820fb5b67293f659c272d16287c28078010684ec545a544b16d5e29ec`

## 次に行う一作業

**執筆＝英語初稿の組版**（WSSE様式LaTeXへの流し込み・図1 TikZ組み込み・文献整形・転記数値の確定recordとの機械照合）。**開発＝順序選択待ち**（候補：縦C合議＝3判定役×複数modelの材料が揃った・**着手時は機械化の穴2候補の同枠縦切りから先行**／RQ2追試／運用集計v8／codex再レビュー任意）。
...
- 直近の関連Test：契約016受入時＝reviewer-launch 106件・契約011対象51件・G30 75件・RQ2装置14件・運搬4件・運用集計25件——各単独終了コード0（2026-08-20実測。正本はGREEN測定・是正測定＝合否は単独実行の終了コード）
- 直近の全Test：禁止認証隔離条件の正規全試験2,668件成功・終了コード0（2026-08-20・契約016受入時のreceipt）
