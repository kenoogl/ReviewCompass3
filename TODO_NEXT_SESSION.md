...
利用者が次の一作業を選ぶ。主な選択肢：(1)外部レビュー機械化の縦切り（機械化目標recordの(a)依頼組み立て器・
(b)prompt品質gate・(c)判定取り込み。送信段が実用化した今、(a)から順が自然）、(2)区切り作業（5段手続きの
手順書化、`.gitignore`仕分け、`IC-HANDOFF-GITIGNORE-RECORD-CANONICAL-001`の仕分け）、(3)候補6以降
（G26残部整理・G28継続回収・G27導入解除）、(4)開発レビュー運搬をHuman中継から本経路へ移す運用判断。
選択後、Claudeが契約候補または作業票の作成から着手する。
