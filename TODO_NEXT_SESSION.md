...
- 現在作業：候補2の境界1は、比較核の先行試験43件が未実装モジュールだけを原因として期待失敗した。試験とRED Evidenceを固定し、同じ試験を変えずに比較核を実装する。
- Task Contract：`TC-RC3-PRODUCT-ONE-DESIGN-ACCEPTANCE-CONFORMANCE-004 / version_3_adopted / boundary_1_red_verified / green_pending`
...
境界1の固定済み試験43件を変更せず、tools/design/one_design_acceptance.pyの比較核だけを実装する。

開始条件：

- 対象試験、RED Evidence、本TODOが意味単位commitへ固定される
- 対象試験43件の内容を変更しない
- CLI入口、安全読込、配布設定には着手しない

完了条件：

- 対象試験43件を単独実行して全件成功し、終了コード0となる
- 既存G08保護対象4fileが基準commitから差分0である
- 製品核とGREEN Evidenceをcommitしてから境界2へ進む

後続作業：境界2の安全読込について、symlink、root外、size、競合変更の先行失敗試験を固定する。
...
- 直近の関連Test：候補2の境界1先行試験43件は未実装モジュールだけを原因として期待失敗、終了コード1。既存G08関連31件は成功、終了コード0。既存G08 2実装fileと2試験fileは基準commitから差分0
