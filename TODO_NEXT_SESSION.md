...
- 現在作業：執筆スレッド＝**論文執筆（WSSE短縮版が締切物）**。英語初稿v1＋文献15件BibTeX＋図1 TikZ源＋§5関連研究3本追補まで固定済み（2026-08-20。表現・非主張等の確定事項は草稿v3付記・v4が正本）。**残作業＝WSSE様式LaTeXへの組版と数値の機械照合**。開発スレッド＝**契約016（モデル選択＋記載照合＋登録定型化・全backend）を採用・実装開始**（2026-08-20。独立確認verified・blocking 0件。契約015は同日受入完了＝3 backend体制）。執筆体制＝執筆スレッドは`docs/paper/`のみ書く・開発スレッドが正本管理（計画v2 §3の分担規則）
- Task Contract：`TC-RC3-PRODUCT-MODEL-SELECTION-CORRESPONDENCE-016 / v2（採用済み・実装開始。採用＝DEC-MODEL-SELECTION-CORRESPONDENCE-CONTRACT-ADOPTION-2026-08-20-V1）`
...
- [契約016候補v2（実装の固定入力）](records/task-contract/2026-08-20-model-selection-correspondence-candidate-v2.md) — SHA-256 `0a4a84032fc7470b88e923ec5508785f81abf6ad3f1bb87b30a3793f1943ddf2`
- [契約016採用判断（独立確認verifiedへ束ね）](records/development/2026-08-20-model-selection-correspondence-contract-adoption-decision-v1.md) — SHA-256 `ef4cd3f8407de6154a19885c854752066c9d89eba4ad944c2f045b5b3b841dc2`
...
**開発＝契約016のRED先行実装**（失敗試験の先行固定→最小実装→既定不変golden・後方互換・両suite緑。**実E2E＝terra指定のcodex起動は利用者の明示指示が別途必要**＝契約§2）。**執筆＝英語初稿の組版**（WSSE様式LaTeXへの流し込み・図1 TikZ組み込み・文献整形・転記数値の確定recordとの機械照合）。

開始条件：

- （開発）契約016 v2採用済み・独立確認verified——満たされている
- （執筆）英語初稿v1（§5追補済み）・図1 TikZ源が固定済み——満たされている

完了条件：

- （開発）契約016 §9-1〜6の対象・関連試験が各単独終了コード0（E2E・完了レビュー・受入は後続）
- （執筆）WSSE様式（LaTeX）で組版・数値照合済みの5頁版一式が`docs/paper/`へcommitされる

後続作業：契約016の残り（§9-7実E2E＝terra初起動→§9-8完了レビュー→§9-9受入）、計画v2 §2.3（フル版§3〜5→§1・2・7→§8）、休止record §4、デプロイ方針record §4〜5、RQ2裁定record v2 §7を参照。
