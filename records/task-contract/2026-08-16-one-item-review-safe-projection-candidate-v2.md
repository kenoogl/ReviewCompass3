...
- supersedes：`records/task-contract/2026-08-16-one-item-review-safe-projection-candidate-v1.md`、SHA-256 `b42232fd0f6a559a680c5447845502a0947b0d96caaaddae208c8bf0c94a2f9b`
- 訂正根拠：`records/development/2026-08-16-one-item-review-safe-projection-v1-independent-review-v1.md`、SHA-256 `b211626ba83409e9a892c202c0903e1363b535dc93b6f390627d42361ba3d33f`
- 訂正範囲：§7.1の停止理由集合の一意化（prepare経路で到達しない`stale_material`を削除し8理由の閉じた列挙とする）
...
停止は全て`ReviewStop(reason)`で、停止元を持たない。prepare経路の2関数が到達し得る理由は§7.1の8種であり、
`stale_material`は範囲外のorganize経路（`validate_results`）専用である。
...
3. いずれかが`ReviewStop(reason)`で停止した場合、`part_stopped`（source `part`・終了コード5）とし、
   ...
   理由が`sensitive_data_remaining`のとき`3`、それ以外`2`とする。転記する理由は、prepare経路の2関数が
   到達し得る次の閉じた8種だけとする：`invalid_arguments`、`invalid_path`、`invalid_schema`、
   `invalid_utf8`、`sensitive_data_remaining`、`size_limit_exceeded`、`unreadable_input`、
   `absolute_path_remaining`。この8種以外の理由（organize経路専用の`stale_material`を含む）は
   `internal_failure`とする。
...
5. §7.1の変換表どおり：G02の機微停止（材料に機微情報候補）が`part_stopped`・`part_reason:
   ...
   `absolute_path_remaining`（材料に絶対path）と`invalid_schema`の各変換も確認する。8種の閉じた集合に
   含まれない理由（`stale_material`相当）を注入した反証で`internal_failure`（終了コード4）となることを確認する。
...
次は本候補を固定commitへ記録し、本候補の作成を担当しなかった別担当が、v1の停止原因1件（停止理由集合の
8種・9種競合）が閉じたことと他境界の退行の有無だけを成果物変更なしで限定再確認する。`開始可`になった後、
利用者へ縮小境界の採用と実装開始を一判断として求める（新契約のため条件付き事前承認は引き継がない）。
