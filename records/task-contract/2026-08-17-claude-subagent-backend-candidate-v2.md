...
- supersedes：`records/task-contract/2026-08-17-claude-subagent-backend-candidate-v1.md`、
  SHA-256 `63efc3be76cbe1987ef7dd6cc84e4d06bcda175da35447abc2b874824b44fe4c`
- 訂正根拠：起草側自己レビュー
  `records/development/2026-08-17-claude-subagent-backend-v1-self-review-v1.md`の
  SR-C12-1（promptの読取り道具名をbackend別差し込みへ）、SR-C12-2（E2Eは同一対象集合の別名依頼へ・
  判定record名衝突の回避）、SR-C12-3（受容根拠は--acceptance-ref入力・実在確認つき）
...
- 固定引数（実行器の設計流用・読み取り専用へ縮小）：`--print`・`--output-format stream-json`・
  ...
  prompt（末尾位置引数）。**promptは縦Bの固定雛形を共通とし、読取り道具名だけをbackend別定義の
  差し込み値にする**（agy=`view_file`は不変・claude-subagent=`Read`。雛形の他の文言は変えない。
  SR-C12-1）。
...
- 起動recordへ宣言tier・受容入力・受容根拠（利用者文言のrecord参照）を記載し、判定record冒頭の
  tier記載（既存）と併せて由来を固定する。受容根拠は起動入力`--acceptance-ref <repo相対path>`として
  渡し、指定fileの実在を起動前に機械確認する（不在は停止。SR-C12-3）。
...
8. 実E2E 1回：利用者の明示指示＋Tier 3受容の明示（`--accept-tier 3`と受容根拠record）の下、subagentで
   実対象1件のレビュー一往復を実環境で行う。対象は**§9-10の完了レビューと同一対象集合（同じdigest表）を
   持つ別名の依頼record**（契約011の正式経路でslug末尾`-subagent`として別途組み立てる。判定record名の
   1対1導出による衝突の回避。SR-C12-2）。組み立て器の雛形は依頼先行を正式経路既定（agy）で記載するが、
   **起動backend・tierの権威はアダプタが起動record・判定recordへ刻印する値**である（契約010
   SR-C10-1の原則）。これにより§9-10のagy判定と並ぶ初の2 oracle比較データを取得する。不成立なら停止し、
   自動再試行・自動切替をしない。
