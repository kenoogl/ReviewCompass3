...
- supersedes：`records/task-contract/2026-08-17-request-builder-candidate-v2.md`、
  SHA-256 `1ef08e6861276c60166255008bf7a86447a7191e12c81fc6b9827d24b5537319`
- 訂正根拠：独立確認判定record（cr-011-001・verified_with_findings・blocking 0件）の所見3件
  （`evasion-secret-matching-exclusion`・`evasion-placeholder-modification`・
  `evasion-empty-digest-table`）。利用者採用「所見3件を採用する」（2026-08-17 chat）。
  v1→v2の訂正根拠（自己レビューSR-C11-1〜3）はv2のとおり
...
1. **類型登録形と雛形生成**：2類型（`contract_review`・`completion_review`）の雛形をcode内定数として
   ...
   （`<<記入:...>>`形式のplaceholderで生成）。出力先pathが既存の場合は`output_already_exists`で
   停止する（new-only書込み。既存fileの変更・削除・上書きを行わない）。
...
3. **機械検査入口（`check`）**：完成した依頼recordへ次を決定的に検査し、不合格は停止理由つきで止める。
   ...
   3. digest表の参照fileの実在・commit済み・記載digestと実digestの全件一致。表が空（0件）の場合は
      `digest_table_empty`で停止する（対象0件の依頼recordを不正とする。cr-011-001所見の反映）
...
`digests.file_sha256`・`reviewer_launch.record.verdict_record_relative_path`（公開関数）・
`reviewer_launch.core.ALLOWED_RESPONSE_MODELS`（公開定数。§5.1-3-5の検査基準として読取りのみ）を
流用し、自前の薄い包み（検査の対象選定・停止理由・除外定数）だけを新設する。既存4製品の包みは
変更しない。
...
- LLM記入欄は`<<記入:`で始まるplaceholderとして生成し、`check`はplaceholder残存を
  `placeholder_remaining`で停止する。検知は完全形だけでなく破損断片（`<<記入`・`記入:`等の残存断片）も
  対象とし、記入欄の内容が空（雛形既定文のまま）の場合も`fill_in_missing`で停止する
  （cr-011-001所見の反映）。
...
自動切り詰めを行わない。`check`は何度でも再実行できる。commit前の実行で
`request_record_uncommitted`だけが不合格となる状態は正常な途中経過であり、**最終合格はcommit済み
状態での全項目合格**とする（検査の複製やmode分岐は作らない）。
...
3. 雛形は実測2通に基づくため、将来の類型追加・構造変更で契約改定が必要になる。緩和：類型登録形が
   追加を局所化する。
4. **既知のすり抜け形の明示受容（cr-011-001所見の反映）**：除外3形式と同形の実鍵（40／64桁小文字hex
   形式の鍵・可読連結名を装う鍵）は高乱雑性検知を通り得る。これは契約009 v2 §7.2で利用者が受容済みの
   既知限界と同一であり、本契約も同じ受容を引き継ぐ。緩和：既定5 pattern検知は不変・対象はcommit済み
   内容だけ・既知すり抜け形の通過を明示する固定試験（§9-5）が限界を可視化し続ける。
...
1. RED：雛形生成（2類型・必須節・placeholder）・digest表計算・機械検査7項目の各停止理由・
   ...
   cr-011-001所見の敵対試験3件——破損placeholder（`<記入:`等）の検知・空digest表の拒否・
   既知すり抜け形の通過明示——を含める。
...
5. 機微検査：既定5 pattern・乱雑列の検知が停止し、40／64桁hexと可読連結名が除外で通過する。
   **除外3形式の自前定数が契約009定数と同値であることを試験で固定する。**除外と同形のダミー鍵が
   通過する事実を明示的に固定する試験（§7.4-4の許容の可視化）を含める。
