...
- supersedes：`records/task-contract/2026-08-16-minimal-operation-contract-execution-candidate-v1.md`、SHA-256 `1ed92a89a96550fe1ea5df74fc40fd74102694e8bfefa07b5ec0c9d09df1bb6d`
- 訂正根拠：`records/development/2026-08-16-minimal-operation-contract-execution-v1-independent-review-v1.md`、SHA-256 `3eb9eba738171ac0f66572de1da5454377684f5ab4d4c110e85397c86657e5ca`
- 訂正範囲：registryの2操作への縮小（一件レビュー部品の除外）と、一時成果・原子的公開によるfile書込み境界の再定義
...
3. **原子的公開**：一時名から最終名へhard linkを作成する（最終名が既存なら失敗し、上書きは起きない）。
   公開後に一時名を削除する。
4. **失敗時の回収**：一時成果の作成後に書込み・照合・公開のいずれかが失敗した場合、自らこの実行で作成した
   一時成果だけを削除して停止する。回収の削除まで失敗した場合に限り一時名のfileが残り得るが、一時名は
   実行記録の正本ではなく、最終名は作成されない。
...
外部送信、任意コード実行、入力外探索を禁止する。削除は手順4の自作一時成果一件だけを例外として許可する。
...
停止時は**最終名の実行記録を作成しない**。一時成果の作成後に停止する場合は§7手順4で回収し、回収の削除まで
失敗した場合に限り一時名のfileが残り得る（一時名は正本ではない）。

処理順は、引数検査、契約読取り、復号、機微検査、schema検査、出力先事前検査、部品実行、束縛照合、
一時成果の作成・照合、原子的公開である。同じ段階では入力順または§8の項目順で最初の違反を返す。
...
| 一時成果の作成・書込み・再読込照合・hard link公開・回収の失敗 | `record_write_failed` | `output` | 4 |
...
16. 一時成果一件と最終名一件（公開後は最終名一件だけ）以外にfile変更がなく、通信、外部process、Git、環境値解決、
    時刻取得、入力外探索を行わない。
16b. 一時成果の作成後に書込み・照合・公開のいずれかを失敗させた反証で、最終名のfileが作成されず、一時成果が
    回収されることを確認する。公開のhard linkが最終名既存で失敗し、上書きが起きないことを確認する。
...
- §6の再利用・保護対象の変更が必要になる。
- 通信、外部process、既存fileの上書き・削除（§7手順4の自作一時成果の回収を除く）、directory作成、環境値解決、時刻取得が必要になる。
...
次は本候補を固定commitへ記録し、本候補の作成を担当しなかった別担当が、v1の停止原因2系統が閉じたことと
他境界に退行がないことだけを成果物変更なしで限定再確認する。`開始可`になった後は、利用者の自律実行指示
（開始可・blocking 0件を条件とする事前承認）に基づき実装へ進み、最終の製品受入だけを利用者判断として残す。
