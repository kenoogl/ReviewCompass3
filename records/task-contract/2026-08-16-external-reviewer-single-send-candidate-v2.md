# 外部レビュア一回送信の最小経路 作業契約候補 v2

- 契約ID：`TC-RC3-PRODUCT-EXTERNAL-REVIEWER-SINGLE-SEND-008`（v1の`TC-RC3-PRODUCT-GEMINI-SINGLE-SEND-008`を改名）
- 契約版：2
- 契約種別：製品処理・G20（外部送信安全境界）の最初の縦切り
- 状態：`candidate_corrected_pending_independent_review_and_human_approval`
- 作成日：2026-08-16
- 直前の製品契約：`TC-RC3-PRODUCT-ONE-ITEM-REVIEW-SAFE-PROJECTION-007 / v2`（受入済み）
- supersedes：`records/task-contract/2026-08-16-gemini-single-send-candidate-v1.md`、SHA-256 `a1cad58471d5928239d64c7a6fdf63211bbfaef199b89f4d57df6709d4e7b627`
- 訂正根拠：2026-08-16の利用者指示「対象はGemini、Codexに加え、操縦LLMがCodexの場合はClaudeも対象となりうる。切り替えられるようにする。切り替えはReviewCompass／ReviewCompass2で実装済」。
  【実測】先行実装`ReviewCompass/tools/api_providers/providers.py`に`anthropic-api`／`openai-api`／`gemini-api`の3 provider切り替え（各`ANTHROPIC_API_KEY`／`OPENAI_API_KEY`／`GEMINI_API_KEY`）が存在する
- 訂正範囲：宛先を3 providerの切り替え式へ変更し、操縦LLMと同一providerへの送信を停止する独立性検査を追加
