# Gemini一回送信の最小経路 作業契約候補 v1

- 契約ID：`TC-RC3-PRODUCT-GEMINI-SINGLE-SEND-008`
- 契約版：1
...
Gemini APIへ一回送信し、未加工応答を保存し、送信台帳を着地させる最初の縦切り**である。
...
- 送信試行recordの着地（送信前）、Gemini APIへの一回のHTTPS送信（再試行なし）、未加工応答の保存、
  結果recordの着地、標準出力への結果一件。

### 5.2 範囲外

- 応答の解析・判定抽出・G02 organizeへの接続（後続。応答は未加工保存まで）。
- 複数送信の一括・並列、自動再試行、送信の取り消し、Gemini以外の宛先。
...
- 宛先hostは`generativelanguage.googleapis.com`だけ。HTTPSだけ。
- modelは`gemini-2.5-pro`、`gemini-2.5-flash`の2値だけ。
- API keyは環境変数`GEMINI_API_KEY`一つだけから読む。**本契約が本repositoryで初めて許可する環境値解決であり、
  この一変数に限る。** keyはHTTP headerでだけ送り、URL・標準出力・台帳・応答保存・例外・logへ一切含めない。
...
送信の**前**に、`{order_identifier}--attempt-v1.json`をoutput_rootへ新規作成専用・一時名＋hard link
公開（契約006 v4 §7と同じ二段）で着地させる。項目：`status: attempt_recorded`、`schema_version: 1`、
`order`（identifier・sha256）、`destination_model`、`purpose`、`source_files`（path・sha256の一覧）、
...
| 送信指示・由来fileの機微情報候補 | `sensitive_data_remaining` | `order`／`source_file` | 3 |
| allowlist外・digest不一致・UTF-8無効の由来file | `source_file_rejected` | `source_file` | 2 |
| payload size超過・件数超過・累計上限到達 | `limit_exceeded` | `order`／`ledger` | 2 |
| 試行record既存（同一IDの再送） | `duplicate_order` | `ledger` | 2 |
| `GEMINI_API_KEY`未設定・空 | `missing_credentials` | `credentials` | 2 |
| output_root不存在・record名既存・書込み失敗 | 契約006 v4 §11と同形 | `output` | 2／4 |
| HTTP応答が200以外（応答は`--response-v1.raw`へ保存済み） | `http_error` | `network` | 7 |
| 送信後の通信例外（応答なし。試行recordは残存） | `network_failure` | `network` | 7 |
| 上記へ分類できない内部例外 | `internal_failure` | `none` | 4 |

## 11. 変更上限

1. 送信核`tools/egress/gemini_send.py`（新規。既存7 moduleは変更しない）。
2. 入口`tools/egress/gemini_send_entry.py`（新規）。`send --order <絶対path>`だけ。
...
2. payload構成が§9の固定形どおりで、由来fileのdigest不一致・allowlist違反・UTF-8無効が停止する。
...
5. 累計上限・size上限・件数上限で停止する。
6. `GEMINI_API_KEY`が結果・台帳・標準出力・例外・応答保存のどこにも現れないことを、鍵値の全出力走査で確認する。
7. 通信は固定host・固定経路への一回だけで、再試行・別経路・URLへの鍵混入がないことを模擬層で確認する。
8. 通信失敗・HTTP異常の各停止で、試行recordが残り、結果recordが作られないことを確認する。
9. 時刻取得が台帳の時刻記録だけに使われ、環境値解決が`GEMINI_API_KEY`だけであることを確認する。
...
- `GEMINI_API_KEY`以外の認証情報・環境値・任意URL・再試行・並列送信が必要になる。
- 送信ごとの人の確認を前提にしないと安全を主張できない欠陥が見つかる。
- 対象、関連、正規全試験または独立確認が不合格になる。

## 14. 影響、未実施、次作業

【判断】受入後は、利用者の指示の下で、commit済み資料によるレビュー依頼をGemini APIへ機械的な守り
