# モデル選択・記載照合・登録定型化（全backend） 作業契約候補 v2
...
- supersedes：`records/task-contract/2026-08-20-model-selection-correspondence-candidate-v1.md`、
  SHA-256 `d5628d16b461b4856f30baa0efd78a51336df9e641ed84544c3618dda5946418`
- 訂正根拠：起草側自己レビュー
  `records/development/2026-08-20-model-selection-correspondence-v1-self-review-v1.md`の
  SR-C16-1（正準抽出核の配置＝循環import回避）、SR-C16-2（golden・後方互換の機械化形）、
  SR-C16-3（正準位置の領域定義）
...
- **検査の強化**：依頼先行を**正準位置**から機械抽出し、backend・model対の所属をそのbackendの
  許可一覧で検査する。抽出核は縦B側の単一実装を共用する（§7.2）。
...
| B 統合（依頼先行を軸にした3点セット＋登録定型化） | 依頼先行のbackend別差し込み・正準位置検査・起動時照合を同じ1行を軸に実装し、選択入力と登録定型化を併せる。抽出核は縦B側に単一実装 | 記載と実行の不一致という過渡状態を作らない。正準行解析・fence状態追跡・許可一覧callable（契約013・015の成果）を流用し、既存の依存方向（request_builder→reviewer_launch）を保つ。既定は全て現行値で互換。推奨 |
...
1. **組み立ての選択入力**：`assemble`へ`--backend`（既定`antigravity-cli`。`BACKENDS`のkey以外は
   ...
   byte同一**（§9-2の既定不変golden）。
2. **依頼先行のbackend別差し込み**（§7.2）：`--backend`既定（agy）は現行文言をbyte不変で維持し、
   agy以外を指定した場合だけbackend別の固定文言で差し込む。
3. **検査の強化（SR-C16-1反映）**：依頼先行の**正準抽出核を`tools/reviewer_launch/core.py`へ
   単一実装**し（依存方向＝request_builder→reviewer_launchの既存importで共用。循環importと
   複製の両方を避ける）、`check`の依頼先行検査を「正準位置の依頼先行からbackend名・model名を
   機械抽出→backend名は`BACKENDS`のkeyと照合→modelはそのbackendの許可一覧と照合」へ強化する。
   文書全体検索（現行`_MODEL_PATTERN.search`）は廃止し正準位置だけを正とする。和集合記号
   `ALLOWED_RESPONSE_MODELS`は互換のため維持する（名称・tuple意味不変）。
4. **起動の選択入力と対応照合**（§7.3）：`launch`へ`--model`任意引数（既定＝一覧先頭・backend
   一覧の所属検査つき）。起動前に同じ抽出核で依頼recordの記載を抽出し、記載backend≠起動backendは
...
- 縦C合議。過去に組み立て済みの依頼recordの書き換え（歴史的record不変。§7.3の移行整理を参照）。
...
### 7.2 依頼先行の正準形・抽出核・backend別差し込み

- **正準位置の定義（SR-C16-3反映）**：先頭見出し行の直後から、最初の`## `節見出し（fence外）
  までの範囲にある**fence外の`- `行群**の中の、**最初の「- 依頼先：」行**だけを正とする。
  範囲内に該当行が無い、または抽出が成立しない場合はfail-closed（起動段は
  `request_backend_mismatch`・検査段は既存`required_statement_missing`）。fence内・本文中の
  同形行では判定しない（文字列理解の原則2。敵対fixtureを§9-1で固定）。
- **抽出核の配置（SR-C16-1反映）**：抽出関数（正準位置の特定＋「最初のbacktick対＝backend名・
  `許可model`直後のbacktick対＝model名」の抽出）は`tools/reviewer_launch/core.py`へ単一実装し、
  request_builderは既存の依存方向のimportで共用する。複製実装を作らない。
- 既定（agy）の行は現行文言をbyte不変で維持する：
  `- 依頼先：Reviewer（第1 backend `antigravity-cli`＝`agy`、許可model `<model>`）`
  （実record実測：backtick対は`antigravity-cli`・`agy`・`<model>`の順で、最初の対がBACKENDSの
  keyと一致＝自己レビュー§3）
- agy以外の差し込み形（契約固定文言）：
...
- 起動核は依頼recordのbyte（digest束縛済み）から§7.2の抽出核で記載を取得し、次を検査する：
  ...
  2. 記載modelがrequested model（`--model`解決値）と不一致 → **`request_model_mismatch`** で
     起動前に停止。
  3. 正準位置から抽出できない → fail-closedとして`request_backend_mismatch`に含める
     （曖昧な救済をしない）。
...
1. **受入済み2製品の同時改修による回帰risk**。緩和：既定不変golden（§9-2）＋両suite無変更部分の
   全緑＋正規全試験。
...
1. `tools/reviewer_launch/core.py`（正準抽出核の単一実装・`--model`受領＝requested解決・
   記載照合・新設2語彙）。
2. `tools/reviewer_launch/entry.py`（launch任意旗`--model`）。
3. `tools/request_builder/core.py`（依頼先行のbackend別差し込み・抽出核の共用による正準位置
   検査・`--backend`／`--model`受領）。
4. `tools/request_builder/entry.py`（assemble任意旗2つ）。
...
2. **既定不変golden（SR-C16-2反映）**：固定入力（`--date`明示・固定fixture対象）での組み立て
   出力のSHA-256を**改修前の現行実装から機械取得してpin**し、`--backend`／`--model`省略時の
   出力が改修後もbyte同一であることを機械証明する。起動のrequested model解決（既定）も現行と
   同一。
3. **後方互換（SR-C16-2反映）**：(a) commit済み実recordの依頼先行の**逐語fixture**で抽出互換
   （backend名・model名が現行recordから正しく取れる）を機械確認し、(b) 既定組み立て（agy）の
   recordが新checkで従来どおり合格する。
...
次は本候補の固定commit後、依頼record組み立て（契約011の正式経路）→機械検査→独立確認（agy
headless起動。起動は利用者の明示指示による）→採用判断の順で進める。
