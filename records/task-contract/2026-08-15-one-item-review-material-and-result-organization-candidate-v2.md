...
- supersedes：`records/task-contract/2026-08-15-one-item-review-material-and-result-organization-candidate-v1.md`、SHA-256 `574b94d6620b2bc57f36a6c84cc8a4ed17d041be096e3f9e41c9589aa9aa32b4`
- 利用者判断：2026-08-15の「では、一件のレビュー材料作成と結果整理に取りかかる」
- 訂正理由：独立定義挑戦が示した、識別子の機微情報検査漏れ、複製結果の一致誤認、出力・絶対位置規則の後決め余地の3原因
- 訂正根拠：`records/development/2026-08-15-one-item-review-task-contract-definition-challenge-v1.md`、SHA-256 `c1ec9fc3dc033c1dbf14c5201966497b1e2c8eae18cd38ededce5e8637ebd4b3`
- 実装状態：未開始
- 危険度：高
...
| supersedes | 候補v1、SHA-256 `574b94d6...aa32b4` |
| 訂正根拠 | 独立定義挑戦、SHA-256 `c1ec9fc3...d4b3` |
...
- schemaで形式を確認したSHA-256欄を除き、資料本文、自由記述、表示名、全てのIDとkeyを含む利用者入力文字列に
...
- UTF-8 decode、固定schema検査、SHA-256欄以外の全利用者入力文字列に対する機微情報候補と絶対pathの検査。
...
schemaで固定したSHA-256欄以外の全利用者入力文字列について、次のいずれかを含めば絶対pathとして停止する。

1. 文字列先頭、空白、引用符、括弧、backtickの直後から始まる`/`と、それに続く一文字以上。
2. 英字一文字、`:`、`\\`または`/`で始まるWindowsのdrive path。英字の大文字・小文字は区別しない。
3. 文字列先頭または空白・引用符・括弧・backtickの直後から始まる`\\\\server\\share`または`//server/share`形式。
   serverとshareは空にできない。`https://`等、schemeの`:`直後にある`//`はこの条件に含めない。
4. 大文字・小文字を区別しない`file://`と、それに続く一文字以上。

相対pathらしい文字列、URLの`https://`、単独の`/`は、この規則だけを理由に停止しない。規則版は本契約版2に固定し、
実装者が検出対象を追加または削除してはならない。
...
5. 既定の秘密pattern、高い乱雑性を持つ長いtoken、§8.6の各絶対pathを、本文、自由記述、表示名、ID、keyへ
   一つずつ入れると、値や一致箇所を出さず停止する。形式・参照一致を確認済みのSHA-256欄だけは除く。
...
13. 成功・停止のJSON bytes、追加key禁止、配列順、末尾LF、空の標準エラーが§8と一致する。
14. 成功・失敗の全経路で、file書込み、network、外部process、Git、環境値解決、権限変更、外部送信が0回である。
15. 正式入口を導入後の場所から実行でき、現在位置に依存しない。
16. 新入口の対象試験、影響する既存G02試験、安全出力に関係する試験、正規全試験が単独commandで成功する。
17. 実装後の独立完了レビューが契約適合と上位目的への影響を確認し、利用者が製品処理として受け入れる。
...
| 絶対path検査 | 本契約v2 §8.6の4形式 | 変更後は契約版を上げ、安全出力試験を再実行 |
