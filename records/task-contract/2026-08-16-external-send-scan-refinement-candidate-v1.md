...
【判断】本契約は、**高乱雑性検知への契約固定の除外3形式の追加（検知呼出し1箇所の変更）と、
...
そのまま引き継ぐ。**検知範囲を狭める本変更の承認は、本契約の採用判断で満たす**（除外の正確な形は§7で
契約固定し、実装者の裁量を残さない）。
...
| B 契約固定の除外3形式 | §7の3形式（正規hex 2種＋可読連結1種）を`find_high_entropy`の公開引数`allow_patterns`へ渡す | 起草時実測で観測3文書とTODO・契約文書が合格し、乱雑列6種の停止が維持された。推奨 |
...
- 送信核の機微検査呼出し（`_scan_text`）への契約固定`allow_patterns`（§7の3形式）の追加。
  送信指示の検査と由来file内容の検査の両方に同一に効く。
- 送信路の改名（§8）。
- 精密化の対象試験（誤検知解消・敵対耐性・境界値）の追加と、既存対象試験の改名追随。
...
- 停止の維持：全小文字乱雑24字・大小混在乱雑・hexだけの断片連結・Base64風・39／41／63／65桁hexの
  乱雑列は引き続き停止する。
- `order_identifier`の仕様（24文字以上の乱雑識別子の停止・契約008 v5 §8）はそのまま働く
  （64hex識別子はX1bで除外されなくなる位置——識別子は`_scan_order`で除外なし検査——ではなく、
  X1b除外の対象になる。**本契約はこの点を変更しない**：識別子には24文字未満の可読形を用いる規約を維持し、
  §10の受入条件で64hex識別子の停止が維持されることを確認する。維持できない場合は§11で停止する）。

**残余risk（明示的に受容を諮る）**：

1. 40桁または64桁ちょうどの小文字hex形式の実鍵は、X1a／X1bを通り得る。
...
1. `tools/external_review/send.py`（gemini_send.pyの改名＋§7の`allow_patterns`定数と`_scan_text`
   1箇所の変更だけ）。
...
1. RED：精密化前の現実装で、観測recordの誤検知（可読file名を資料とする送信指示の
   `sensitive_data_remaining`停止）を再現する試験が失敗し、§7適用後に合格する。
2. 誤検知の解消：§7の起草時実測（観測3文書・record file名・TODO・契約文書の合格）を試験で再現する。
3. 停止の維持（敵対）：§7記載の乱雑列6種と、40／64桁の前後1桁（39・41・63・65桁）の乱雑hexが
   引き続き停止する。64hexの`order_identifier`が引き続き停止する（識別子規約の維持）。
4. 既定5 pattern（AWS ID・Bearer・代入形式・秘密鍵ブロック・email）の検知が全て維持される。
...
- §7の3形式で誤検知解消と乱雑列停止の両立が固定できない（形式の追加・変更が必要になる）。
- `order_identifier`の乱雑識別子停止の仕様が維持できない。
...
次は本候補の起草側自己レビュー（5段手続きの第1・2段）を行い、暫定体制（Gemini・Human中継）の
独立確認へ運搬してもらう。`開始可`の後、利用者へ採用と実装開始を一判断として求める。
