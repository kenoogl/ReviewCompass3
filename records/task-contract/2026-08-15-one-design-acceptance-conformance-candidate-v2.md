# 一件の設計・受入条件照合 作業契約候補 v2

- 契約ID：`TC-RC3-PRODUCT-ONE-DESIGN-ACCEPTANCE-CONFORMANCE-004`
- 契約版：2
- 契約種別：製品処理
- 状態：`candidate_corrected_pending_independent_definition_review_and_human_approval`
- 作成日：2026-08-15
- 直前の製品契約：`TC-RC3-PRODUCT-ONE-ITEM-REVIEW-003`
- supersedes：`records/task-contract/2026-08-15-one-design-acceptance-conformance-candidate-v1.md`、SHA-256 `1640ebbfd1ff5d01e4410b43de6c503da8dd0b402bc47d4f96534cbcdf71f52f`
- 訂正根拠：`records/development/2026-08-15-one-design-acceptance-contract-internal-challenge-v1.md`、SHA-256 `883c9f029253c3221dbdb18a734e92d6ce3948c07135143f3883c26f0abbf2ad`
- 訂正範囲：条件subject一意、利用時点の安全読取り、停止元の安全表示だけ
- 利用者判断：2026-08-15の「次へ」と、残る7候補の自律実行指示
- 実装状態：未開始
- 危険度：中
- 内容識別値：本候補を固定した後、独立定義確認と利用者判断記録から参照する

ここでいう「作業契約」は、一つの仕事について目的、入力、範囲、許可操作、期待結果、確認方法、停止条件を
実装前に固定する約束である。「内容識別値」は、検査・正規化後の正準JSONからSHA-256で計算する改変検出用の値である。
...
正しさを推測しない。最初の一件では、各`subject`を設計と受入条件でそれぞれ一意にし、受入条件同士の競合を
設計側の矛盾へ混ぜない。
...
【提案】案Cを採用候補とする。既存機能だけで済む案Aは目的を満たさず、案Bは一件照合に不要な広い責務を持ち込む。
...
- 固定schema、識別子、値型、重複、件数、root内束縛を検査する。
...
- 二入力は解決後にroot内にあり、互いに異なる通常fileである。
- rootからfileまでの全構成要素にsymlinkがない。
...
- UTF-8復号、JSON復号、固定schema、件数、型、識別子の検査。
- 正準JSON、SHA-256、型付き完全一致、集合の包含・非交差、件数、並べ替えの決定的計算。
- 入力値を含まない正常結果または停止結果のJSON一件の標準出力。

入力rootをdirectory file descriptorとしてsymlink非追跡で開く。各fileはrootから相対的に、全構成要素を
directory file descriptorから`O_NOFOLLOW`相当で開く。open後に通常fileであること、size、機器番号とinodeを確認し、
上限より1 byte多く読める処理で上限超過を検出する。読取り後に再度種類、size、機器番号、inodeを確認し、
open後の値または実読取りbyte数と一致しなければ`unreadable_input`で停止する。この方法を使えない環境では停止する。
`resolve()`後の文字列比較だけで安全とみなさない。
...
文字列は大文字小文字を区別する。文字列配列は入力順を意味にせず、文字列昇順へ正規化する。
異なるJSON型の値を等しいと扱わない。
...
| `contains_all` | 両値が文字列配列で、期待値の全要素が設計値に含まれる |
| `contains_none` | 両値が文字列配列で、共通要素がない |

`contains_all`と`contains_none`の期待値が文字列配列でなければschema不正とする。同じ`subject`の設計値が
文字列配列でない場合は入力不正ではなく当該条件の`矛盾`とする。条件は`condition_id`順へ正規化する。
...
2. 設計事実があり、§6.4の比較が成立すれば`satisfied`。
...
終了コード0で、入力の自由値と絶対pathを含まず、次の項目だけを持つ正準JSON一件を返す。
...
`satisfied`、`unreferenced_fact`の順で、各項目は`kind`と昇順の`identifiers`だけを持つ。0件の区分は省く。
満たした条件も人の判断一覧から外さない。
...
成功成果を返さず非0終了コードとし、次の項目だけを持つ正準JSON一件を返す。
...
`source`は`arguments`、`design`、`acceptance`、`none`だけとする。引数不正とpath不正は`arguments`、
設計fileだけに確定した読取り・size・UTF-8・schema不正は`design`、受入条件fileだけなら`acceptance`、
入力fileを特定できない読取り失敗と内部失敗は`none`とする。値、項目名、path、例外本文は返さない。

固定理由は`invalid_arguments`、`invalid_path`、`unreadable_input`、`size_limit_exceeded`、`invalid_utf8`、
`invalid_schema`、`internal_failure`だけとする。引数・path・読取り・size・UTF-8・schemaは終了コード2、
内部失敗は4とする。
...
9. 未知項目、重複ID・subject、不正識別子、禁止値型、空配列、件数超過を成功にしない。
10. root外、全構成要素のsymlink、通常file以外、同じ二入力、読取不能、size超過、UTF-8不正を成功にしない。
...
本候補を固定commitへ記録し、別実行単位が成果物を変更せず定義を反証する。目的の縮め過ぎ、4比較と型、
重複subject、利用時点読取り、停止元表示、内容識別値、既存G08非変更、禁止作用へ反例を試す。
