# codex-cli第3 backend（登録簿深化同時実施） 作業契約候補 v2
...
- supersedes：`records/task-contract/2026-08-20-codex-cli-backend-candidate-v1.md`、
  SHA-256 `91271f51299e435c0cce191e0226a21602dec827b7160f952583e668f6c4ae14`
- 訂正根拠：起草側自己レビュー
  `records/development/2026-08-20-codex-cli-backend-v1-self-review-v1.md`の
  SR-C15-1（読取り指示ブロックの差し込み単位の定義）、SR-C15-2（repo外読取り遮断の未保証を
  残余riskへ計上）、SR-C15-3（stdin遮断＝起動核の共通固定事項）、SR-C15-4（schema一時fileの詳細）
...
- **backend登録簿の深化**：backend定義へ「読取り指示ブロック・引数組み立て関数・stream解析
  （model観測・判定抽出）関数・許可model一覧・禁止環境変数一覧・通過変数一覧・子環境注入・
  project束縛要否」を吊るし、name分岐6箇所を登録参照へ置換する。agy・claude-subagentの現行値は
  **不変のまま移設**する。
...
1. **backend登録簿の深化**：backend定義を「provider・executable・宣言tier・読取り指示ブロック・
...
2. **codex-cli backendの追加**：provider `openai`・executable `codex`・宣言Tier 1。読取り指示
   ブロックは§7.2の契約固定文。
3. **起動固定形**（§7.2）：`exec` subcommand・`--json`・`--output-schema`（第一候補）・
   `--sandbox read-only`・`--skip-git-repo-check`・`--ephemeral`・`--ignore-user-config`・
   `-m <許可一覧先頭>`・prompt末尾位置引数・作業dir＝対象repository。stdin遮断は起動核の共通
   固定事項（§7.2）。
...
- 転記・事後照合・保存・G30登録・導線の変更（既存流用）。外部API直接送信経路の後続（pendingのまま）。
  歴史的recordの書き換え。prompt雛形の共通骨格の文言変更（§7.2の読取り指示ブロックの差し込み
  一般化を除く）。
...
- 固定引数：`exec`・`--json`・`--output-schema <schema一時file>`・`--sandbox read-only`・
  ...
  prompt（末尾位置引数）。作業dirは対象repository（`-C`は使わない）。
- **stdin遮断（起動核の共通固定事項）**：起動核はsubprocessへstdin遮断を常時適用する（全backend。
  headless起動の前提。疎通実測§7でcodex execがstdinを読む挙動を観測した。既存試験・生成prompt・
  引数byteへ影響しないことは自己レビューSR-C15-3で機械確認済み）。
- **schema一時file**：既存`VERDICT_SCHEMA`（無変更・共用）の正準JSONを、起動核がOSの一時領域
  （`tempfile`）へ機械書き出しし、そのpathを渡す。内容は契約固定で毎回同一byte。run毎の可変部は
  このpathのみで、起動recordの`arguments`欄に残る。
...
- **読取り指示ブロックの差し込み（SR-C15-1）**：prompt雛形のうち、(a) 実行環境の説明、(b) pathの
  制約、(c) 最初の操作とfreshness指示——の**連続3文ブロック**だけをbackend別差し込みとし、雛形の
  他の文言（Reviewer宣言・対象表示・期待SHA-256・構造化出力指示・unexamined指示）は共通骨格として
  不変とする。agy・claude-subagentのブロックは**現行文言の逐語移設**（道具名`view_file`／`Read`の
  差し込みを含めて生成promptのbyte不変をgolden試験で機械証明）。codex用ブロックは次の契約固定文と
  する（RED段実測後の微修正は契約訂正として扱う）：

  > この実行環境は読み取り専用のsandboxです。読み取り専用のshell command（`cat`・`ls`・
  > `shasum`等）だけを使い、fileへの書込み・変更・networkアクセス、および対象repositoryの外
  > （利用者のhome等）のfile読取りを一切行わないでください。
  > 読取りには対象repository配下の絶対pathだけを使ってください。
  > 最初の操作として、`cat`で対象依頼recordの絶対pathを開いて本依頼の対象であることを確認し、
  > `shasum -a 256`で期待SHA-256との一致を確認してください。digestの計算がこの環境で行えない
  > 場合、freshnessにはexpectedへ期待SHA-256を転記し、resultをnot_computableとして理由を記載して
  > ください（内容が明らかに別物ならmismatchとして判定せず停止）。
...
5. 登録簿深化はagy・claude経路の内部構造変更を含む。緩和：生成prompt・引数のbyte不変golden＋
   既存試験の無変更全緑の機械証明（§9-2）。
6. **repository外読取りの機械遮断は未保証（SR-C15-2）**：codexのread-only sandboxは書込み・承認
   迂回の遮断が主であり、repo外の読取りが機械遮断される保証はhelpからは確認できない。緩和：
   §7.2の読取り指示ブロックで禁止を指示し、実E2Eのraw（道具実行の記録）で領域外読取りの有無を
   点検して記録する（§9-8）。露出の実体は本節1と同じ。
...
3. `tools/reviewer_launch/record.py`（codex backend名の転記対応が必要な場合の最小変更。
   `_backend_provider`は`provider`鍵の維持で無変更の見込み＝自己レビュー§3）。
...
1. RED：登録簿一般化（name分岐の消滅）・codex固定引数（読み取り専用・危険旗不在の両向き）・
   ...
   所属検査）・prompt byte不変golden（読取り指示ブロックの逐語移設）の失敗試験を先に固定する。
...
8. 実E2E 1回：利用者の明示指示の下、codex-cliで実対象1件のレビュー一往復を実環境で行う。対象は
   ...
   よる衝突を回避＝契約012 SR-C12-2の型）。完走・raw保存・判定record転記・事後照合4点合格。あわせて
   rawの道具実行記録から**領域外読取りの有無を点検して記録**する（§7.5-6）。不成立なら停止し、
   自動再試行・自動切替をしない。
...
11. 利用者が§7.5残余risk 6点を確認して製品処理を受け入れる。受入をもって
...
次は本候補の固定commit後、依頼record組み立て（契約011の正式経路）→機械検査→独立確認（agy headless
起動。起動は利用者の明示指示による）→採用判断の順で進める。
