# Session記録と伏字化結果の安全保存・再読込み Task Contract候補 v2

- 契約ID：`TC-RC3-PRODUCT-SESSION-ARTIFACT-SAFE-STORAGE-002`
- 契約版：2
- 契約種別：製品処理
- 状態：`candidate_corrected_pending_one_time_delta_review_and_human_approval`
- 作成日：2026-08-15
- prior contract：`TC-RC3-PRODUCT-G25-SESSION-ARTIFACT-PREPARATION-001`
- supersedes：`records/task-contract/2026-08-14-session-artifact-safe-storage-candidate-v1.md`、SHA-256 `6d0f4984b3144d840b69544c70a71a48a29fe11bcdeeaa0075bddc9e8027a57a`
- 訂正根拠：`records/development/2026-08-14-session-artifact-safe-storage-task-contract-definition-challenge-v1.md`、SHA-256 `cce6fb9a94da7cfe81541ebdccf8d0707eb07041f775b3c59969f5e8b937fd65`
- 利用者判断：2026-08-15の選択「1」。契約候補v2の限定訂正だけを承認し、契約採用と実装開始は承認していない
- 実装状態：未開始
- 危険度：高
...
| `supersedes` | 候補v1、SHA-256 `6d0f4984...7a57a` |
| 訂正根拠 | 独立定義挑戦、SHA-256 `cce6fb9a...fd65` |
...
- 初期対応OSはmacOSである。各rootと記録directoryは実効利用者が所有しmode 0700相当、通常fileは同じ利用者が所有しmode 0600相当でなければならない。group・otherまたは追加ACLに所有者以外への権限があれば停止する。
...
- `sensitive_root/<record_id>/`の固定file `operation.json`と`raw.bin`への書込み・再読込み。
- `data_root/<record_id>/`の固定file `operation.json`、`derived.json`、`manifest.json`、`commit.json`、`deleted.json`への書込み・再読込み。
- 上記固定fileと同じdirectory内の一時file、同一root内の名前変更、必要な同期。
...
一記録の固定fileは§7に列挙したものだけとする。両rootの`operation.json`は同じ操作識別値、記録ID、期待する
raw・派生物・manifestのSHA-256、保持期限、状態を持ち、本文とpathを持たない。状態は次の優先順位で判定する。

1. `deleted`：本文fileと`commit.json`がなく、有効な`deleted.json`がある。
2. `deleting`：どちらかの`operation.json`が`deleting`である。通常読込みを必ず拒否する。
3. `committed`：`commit.json`が最後に作成され、両rootの操作識別値と全内容識別値が一致し、`deleting`でない。
4. `incomplete`：当該記録の固定fileが一つ以上あるが、上記三状態を満たさない。

異なる状態が二rootに見える場合は、成功側へ推測せず`deleting`、次に`incomplete`を優先する。root全体を走査せず、
明示された`record_id`の固定pathだけを読む。

### 8.3 保存成功

保存は両rootへ同じ`incomplete`操作情報を置き、raw、保存用派生物、manifestを各root内の一時fileから確定fileへ
移し、全fileを再読込み照合した後、最後にdata側へ`commit.json`を置く。二rootを同時確定できるとは主張しない。
...
同じ入力による`store`再試行は、存在する固定fileが期待する操作識別値と内容識別値に一致する場合だけ再開し、
不一致または予期しないfileがあれば何も変更せず停止する。

保存を取りやめる場合は、`plan-delete`が`incomplete`状態にも対応する。一方または両方に存在する有効な
`operation.json`と固定fileを記録IDから直接確認し、それらへ結び付く削除確認値を作る。確定印や片方の
`operation.json`がなくても、残る操作情報の記録ID・操作識別値・期待する内容識別値が一致すれば、当該途中記録だけを
`delete`で処理できる。有効な操作情報が一つもなければ推測で削除しない。途中fileも保持期限の対象で、期限後も
自動削除せず通常利用を拒否する。
...
manifest識別値があればその値、削除対象の固定fileの種類と件数、削除後監査の扱いを返す。pathは返さない。
削除確認値は、この計画全体の正準JSONへ結び付ける。`deleted`では、保持期限以後の`deleted.json`と残った
`operation.json`だけを対象にでき、本文fileを新たに対象へ加えない。
...
両方が`deleting`になった後に、raw、保存用派生物、manifest、`commit.json`を削除する。削除確認値と操作情報は
...
12. 確定印のない途中状態を通常読込みへ出さず、同じ記録IDの`plan-delete`と確認済み`delete`で中止できる。
...
17. 削除対象は当該記録の固定fileだけで、root、他記録、予期しないfileを削除しない。
18. 成功、失敗、再読込み、削除の標準出力に元記録本文、Session入力path、絶対path、例外本文、秘密値を含めない。
19. 保存・再読込み・削除でnetwork、外部process、Git、環境値解決を行わない。
20. 対象試験、正式入口とG25の関連試験、影響するG26試験、正規全試験が単独commandで成功する。
21. 利用者が合成例の保存、再読込み、途中状態の中止、削除計画、削除再試行を確認し、製品処理として受け入れる。
...
| superseded candidate | 候補v1、SHA `6d0f4984...7a57a` | 履歴として保持し、現在の採用根拠にしない |
| correction review | 独立定義挑戦、SHA `cce6fb9a...fd65` | 指摘四点の変更点レビューまで有効 |
...
1. 本v2を先行指摘四点だけの一回限り変更点レビューへ掛ける。
...
本v2の変更点レビュー後、利用者が次を別々に判断する。

1. v1の止める指摘四点が解消した本v2をTask Contractとして採用するか。
...
【判断】本v2の作成承認は、上記の契約採用、実装開始、G26の正式化を承認したことにはしない。

## 17. 未実施

【未実施】本v2の変更点レビュー、契約採用、実装開始、コード・試験・設定・`pyproject.toml`の変更、
