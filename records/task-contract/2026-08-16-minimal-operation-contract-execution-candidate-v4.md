...
- supersedes：`records/task-contract/2026-08-16-minimal-operation-contract-execution-candidate-v3.md`、SHA-256 `d97a742bd2c67946f4336a06167767c4060a38157073e13cb42e5ecbf2117f85`
- 訂正根拠：実装開始後の失敗試験で発見した契約定義欠陥。固定操作名`requirement_candidate_check`（27文字）が§8.2の高乱雑性検査（既定：24文字以上・乱雑さ3.5以上）に実測3.63で一致し、契約どおり実装するとG24操作の正例が必ず機微停止する。実装の誤りではなく§8.2の除外規則の穴である
- 訂正範囲：§8.2へ`/operation`位置の固定registry操作名だけを検査対象から外す第二の除外を追加し、受入条件10を同じ定義へ合わせる
- 利用者判断：2026-08-16の運用化目標の指示、G30契約定義への着手指示、本線1-4の自律実行指示
- 実装状態：未開始
- 危険度：高
...
- G02の`prepare`・`organize`操作（部品結果に入力自由文が含まれ、入口署名と停止形式も本契約の共通形と異なる。
  自由文を実行記録へ入れない安全な投影、標準出力の捕捉方法、停止元の固定変換を別の後続契約で定義してから追加する）。
- G25取り出し、安全保存との統合、暫定pipeline（bootstrap／egress）との接続。
...
registryは次の2操作へ固定する。各入口は`main(arguments, *, output)`形式の受入済み関数（両入口とも実測で同署名）
であり、変更しない。部品の呼出しは同一process内で行い、subprocessを使わない。両部品の正常・停止結果は入力自由文・
絶対path・例外本文を含まない安全表示であり、停止結果は`reason`と`source`を持つ。
...
実行記録の着地は、一時成果と上書き不能な原子的公開の二段で行う。

1. **事前検査**：`output_root`が既存directoryであり、最終名`{contract_identifier}--execution-v1.json`と
   一時名`{contract_identifier}--execution-v1.json.partial`のどちらも存在しないことを確認する。
2. **一時成果**：一時名のfileを新規作成専用（`O_CREAT | O_EXCL`相当）で作成し、実行記録bytesを書き、閉じ、
   再読込して書いたbytesとの完全一致を確認する。
3. **原子的公開**：一時名から最終名へhard linkを作成する（最終名が既存なら失敗し、上書きは起きない）。
   **hard linkの成功を公開の確定点とする**。確定点以降、最終名は有効な実行記録の正本である。
4. **公開前の失敗時の回収**：一時成果の作成後、公開の確定点より前に書込み・照合・hard link作成のいずれかが
   失敗した場合、自らこの実行で作成した一時成果だけを削除して停止する。回収の削除まで失敗した場合に限り
   一時名のfileが残り得るが、一時名は実行記録の正本ではなく、最終名は作成されない。
5. **公開後の清掃**：確定点の後に一時名を削除する。この削除が失敗した場合は`partial_cleanup_failed`で停止する。
   このとき最終名（公開済みの正本）と一時名（最終名と同一inode・同一bytes、正本ではない）の両方が残る。
   回復境界：一時名の削除は利用者が行う。実行記録は既に有効であり、同じ契約の再実行は最終名既存の
   `invalid_output_root`で止まる。

許可する能力は、契約一fileの読取り、UTF-8復号、JSON検査、機微情報候補検査、registry部品の同一process呼出し、
正準JSON、SHA-256、一時成果一件の新規書込み・再読込・削除、最終名へのhard link作成、標準出力だけである。

既存fileの上書き・削除・改名、directory作成、通信、外部process、Git、環境値解決、時刻取得、乱数、権限変更、
外部送信、任意コード実行、入力外探索を禁止する。削除は手順4・手順5の自作一時成果一件だけを例外として許可する。
...
3b. 正確な位置`/operation`にあり、§6.1の固定registry操作名と完全一致する値だけを検査対象から外す。
   操作名は本契約が公開定義する固定語であり、利用者の機微情報を運び得ない。完全一致しない値は除外せず検査する
   （根拠：`requirement_candidate_check`は27文字で高乱雑性検査の既定と実測3.63で一致するため、除外しないと
   G24操作の正例が必ず停止する）。
...
- `operation`：`design_acceptance_check`、`requirement_candidate_check`だけ。
- `input_root`：絶対path。
...
公開の確定点（§7手順3のhard link成功）より前の停止では**最終名の実行記録を作成しない**。一時成果の作成後に
停止する場合は§7手順4で回収し、回収の削除まで失敗した場合に限り一時名のfileが残り得る（一時名は正本ではない）。
唯一の例外は`partial_cleanup_failed`である。これは確定点の後の停止であり、最終名は公開済みの正本として残り、
一時名（同一inode）も残る。

処理順は、引数検査、契約読取り、復号、機微検査、schema検査、出力先事前検査、部品実行、束縛照合、
一時成果の作成・照合、原子的公開、公開後の清掃である。同じ段階では入力順または§8の項目順で最初の違反を返す。
...
| `output_root`が存在しない・directoryでない、最終名または一時名のfileが既存 | `invalid_output_root` | `output` | 2 |
| 部品が非0の終了コードで停止した | `part_stopped` | `part` | 5 |
| 束縛照合の不一致（`expected_sha256`≠`reported_sha256`） | `binding_mismatch` | `contract` | 2 |
| 公開の確定点より前の一時成果の作成・書込み・再読込照合・hard link作成・回収の失敗 | `record_write_failed` | `output` | 4 |
| 公開の確定点の後の一時名削除失敗（最終名は公開済みの正本として残る） | `partial_cleanup_failed` | `output` | 6 |
...
1. 2操作それぞれの正例で、部品実行、束縛照合、実行記録の着地、標準出力一致を一度ずつ示す。
...
10. AWS鍵形式、email、bearer token、API key代入、秘密鍵block、高乱雑性tokenを契約のkey・値で停止し、
    正しい`expected_bindings`のSHA-256値と`/operation`位置の固定registry操作名だけを検査から外す。
    `requirement_candidate_check`を含む2操作の正例が機微停止しないことと、registry操作名と一致しない
    `/operation`値が除外されないことを確認する。
11. §6.2のfileの内容識別値、公開関数2名、既定pattern件数5を実行前後に照合し、環境依存規則を解決しない。
12. 契約fileのsymlink、非通常file、size超過、UTF-8不正、読取り中変更を停止する。
13. `output_root`不存在・非directory・最終名既存・一時名既存を`invalid_output_root`で停止し、上書きが起きない。
14. 部品停止時は`part_stopped`で部品の`reason`・`source`・終了コードだけを転記し、最終名・一時名のfileを作らない。
15. §11の全行について`reason`、`source`、終了コード、空stderr、値・path・例外非表示を確認する。
16. 一時成果一件と最終名一件（正常完了後は最終名一件だけ、`partial_cleanup_failed`の場合だけ同一inodeの
    一時名も残る）以外にfile変更がなく、通信、外部process、Git、環境値解決、時刻取得、入力外探索を行わない。
16b. 一時成果の作成後、公開の確定点より前に書込み・照合・hard link作成のいずれかを失敗させた反証で、最終名の
    fileが作成されず、一時成果が回収されることを確認する。公開のhard linkが最終名既存で失敗し、上書きが
    起きないことを確認する。
16c. 公開の確定点の後の一時名削除を失敗させた反証で、`partial_cleanup_failed`・終了コード6・空stderrとなり、
    最終名が公開済みの正本として残り、一時名が同一inodeで残り、同じ契約の再実行が`invalid_output_root`で
    止まることを確認する。
17. 配布後の正式実行名を別の現在位置から実行しても同じbytesを返す。
18. §6.1の再利用4 fileが固定内容識別値と一致し、次の各単独commandが成功する。
...
19. §6.3の保護対象が基準commitから差分0で、次の各単独commandが成功する。
...
20. 対象、関連、正規全試験を各単独終了コード0で成功させ、固定commitを別担当が誤合格・未接続・禁止作用・
    上位目的への悪影響0件として確認する。
21. 合成一件で2操作の実行記録、束縛照合、判断待ち、安全表示を示す。
...
- §6の再利用・保護対象の変更が必要になる。
- 通信、外部process、既存fileの上書き・削除（§7手順4・手順5の自作一時成果の回収・清掃を除く）、directory作成、環境値解決、時刻取得が必要になる。
...
次は本候補を固定commitへ記録し、本候補の作成を担当しなかった別担当が、v3実装中に発見した停止原因1件
（固定操作名が高乱雑性検査へ一致する§8.2の穴）が§8.2手順3bで閉じたことと他境界に退行がないことだけを
成果物変更なしで限定再確認する。`開始可`になった後は、利用者の自律実行指示（開始可・blocking 0件を条件と
する事前承認）に基づき実装を再開し、最終の製品受入だけを利用者判断として残す。
