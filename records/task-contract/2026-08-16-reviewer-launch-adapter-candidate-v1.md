# Reviewer起動アダプタ（第1 backend：Antigravity CLI） 作業契約候補 v1
...
2. agy起動部：固定引数（§7.1）による1回起動、認証遮断（§7.1の禁止環境変数）、最小環境渡し、
   `--print-timeout`、終了コード・stream出力の機械取得。
...
4. 独立性tierの機械判定＋宣言：pilot provider（`anthropic`固定）とbackend providerの比較。Tier 1以外は
   `reviewer_not_independent_tier`で停止。判定結果（tier・model）を起動recordと判定recordへ記録。
5. 起動record・未加工出力の不変保存：`tools/bootstrap/raw_review_store.py`の`store_raw_executions`を
   再利用し、repo外私有領域へ保存（保存処理を複製しない）。
6. 判定recordの機械転記：schema適合JSONから`records/session-handoffs/`へ判定recordを生成し、
   そのrecord 1件だけを単独commitする。冒頭へReviewer（provider・model）・tier・起動方式
   （headless機械起動）・未加工出力の保存先種別とSHA-256を記載する。
...
- 固定引数：`--print`・`--output-format stream-json`・`--json-schema <判定schema>`・`--model <許可model>`・
  `--disable-slash-commands`・`--print-timeout <契約固定値>`。`--dangerously-skip-permissions`と
  書込みを許す`--mode`は使用禁止。sandbox・作業ディレクトリの扱いはRED段の実測で確定し、
  読み取り専用相当が成立しない場合は停止する（§10）。
- 認証遮断：子環境から`GEMINI_API_KEY`・`GOOGLE_API_KEY`・`GOOGLE_GENAI_API_KEY`・
  ...
  型を流用。禁止一覧の最終形はRED段の実測で契約固定し、緩めない）。認証は利用者のagyログイン状態だけを使う。
- 許可model一覧：実E2E前に利用者承認で固定し、応答streamのmodel表示を一覧と照合する（実行器の型）。
...
- `reviewer`（provider・model）・`tier`（本契約では`1`固定）
- `freshness`（期待SHA-256・観測SHA-256・`match|mismatch`。mismatchなら判定せず停止報告）
...
1. 鮮度：`freshness`が`match`で、アダプタ自身が再計算した依頼recordのSHA-256と一致する。
...
5. 転記：schema適合の判定JSONから判定recordが生成され、そのrecord 1件だけの単独commitになり、冒頭に
   Reviewer・model・tier・起動方式が載る。schema不適合は転記せず停止し、未加工出力は保存される。
...
次は本候補の固定commit後、5段手続き（自己レビュー→文脈整理→依頼作成→依頼点検）を経て、暫定体制
（Gemini手動・Human中継）の独立確認を受け、利用者の採用判断を求める。
