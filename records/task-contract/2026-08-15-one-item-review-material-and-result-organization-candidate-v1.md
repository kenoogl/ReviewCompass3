...
- 資料本文と、条件・結果の自由記述欄に固定した機微情報候補または絶対pathがあれば、処理は伏字化して続行せず停止する。
- 本検査は全ての機微情報を発見できるとは主張しない。既定pattern、高い乱雑性を持つ長いtoken、POSIXとWindowsの
  絶対pathを検査し、検査限界を出力形式の版と結ぶ。
- schemaで位置と形式が固定されたSHA-256欄は、64桁の小文字16進数であることと参照値への一致を先に検査し、
...
- UTF-8 decode、固定schema検査、固定された機微情報候補と絶対pathの検査。
...
終了コード0で正準JSON一件を返す。最低限、次を含む。

- `status: material_prepared`
- `schema_version: 1`
- 資料の表示用識別子、本文、本文のSHA-256、行数。
- 目的、確認基準、制約、条件JSONのSHA-256。
- 結果入力schemaの版と、共通`issue_key`が整理の根拠になることの明示。
- 材料全体の`material_package_sha256`。
- `external_send_approved: false`。

入力の絶対path、root、file名の自動抽出値、機微情報検査の一致箇所、環境値を返さない。
この出力は外部送信の承認ではなく、利用者の手元で確認する固定材料である。
`material_package_sha256`は、その欄だけを除いた正常結果の正準JSON bytesから計算する。基準はID順に並べる。
...
未知key、別材料の内容識別値、未知の基準、範囲外の行、重複ID、空の結果集合を拒否する。

### 8.4 `organize`の正常結果

終了コード0で、資料本文を含まない正準JSON一件を返す。最低限、次を含む。

- `status: results_organized`
- `decision_status: pending_human_decision`
- `material_identifier`、`material_package_sha256`、資料本文SHA-256。
- 結果集合と各reviewのSHA-256、担当別`verdict`、`summary`、指摘件数。
- `review_count`、`finding_count`、`insufficient_evidence_reviewers`。
- `issue_key`順の指摘群。各群に全reporter、元finding ID、severity、title、description、基準ID、行範囲を残す。
- 同じ`issue_key`が一件だけなら`single_report`。
- 同じ`issue_key`が複数結果にあり、指摘本文と区分が全て同じなら`corroborated`。
- 同じ`issue_key`が複数結果にあり、いずれかが異なれば`conflict`。
- 異なる`issue_key`で、severity、title、description、基準ID、行範囲が完全一致する組を`possible_duplicate_keys`。
- `unresolved_issue_keys`には`single_report`と`conflict`を全て含める。
- `human_decision_queue`には、証拠不足、競合、重複可能性、単独報告の順で、人が確認する対象IDと理由だけを並べる。
  元資料やレビュー本文を再掲しない。
...
`corroborated`の一致対象は、severity、title、description、基準ID集合、開始行、終了行の全てとする。
finding IDとreporter IDは一致対象に含めない。

【判断】`corroborated`は複数の結果が同じ入力を返した事実であり、指摘が正しいという合格判定ではない。
...
5. 既定の秘密pattern、高い乱雑性を持つ長いtoken、POSIX・Windows絶対pathを資料、条件、結果の各入力へ一つずつ入れると、
   値や一致箇所を出さず停止する。
6. `organize`は結果1件から8件までを受け、担当別の結論、要約、指摘件数、元結果の内容識別値を失わない。
7. 空の結果集合、別材料へ結ばれた結果、未知key、未知基準、範囲外行、矛盾した`verdict`と指摘件数を拒否する。
8. 同じ`issue_key`の同一報告を`corroborated`、相違を`conflict`、一件だけを`single_report`へ決定的に分ける。
9. 異なるkeyの完全一致だけを`possible_duplicate_keys`とし、意味が似るだけの指摘を自動統合しない。
10. `single_report`、`conflict`、`insufficient_evidence`を隠さず、採否を自動確定しない。
11. `organize`の正常出力に資料本文、入力path、root、秘密候補、入力にない判断が含まれない。
12. 成功・失敗の全経路で、file書込み、network、外部process、Git、環境値解決、権限変更、外部送信が0回である。
13. 正式入口を導入後の場所から実行でき、現在位置に依存しない。
14. 新入口の対象試験、影響する既存G02試験、安全出力に関係する試験、正規全試験が単独commandで成功する。
15. 実装後の独立完了レビューが契約適合と上位目的への影響を確認し、利用者が製品処理として受け入れる。
...
| 絶対path検査 | 一件用の製品核に閉じた規則版を実装時に固定 | 変更後は安全出力試験を再実行 |
