...
【実測】原因は、対象Issueを更新した後に`record_file.parent.mkdir(...)`を実行し、この処理が解決記録の
書込みと復元を囲む`try`の外にあることである。既存の`test_record_write_failure_restores_issue_without_partial_record`は、
親フォルダが既に存在する状態でファイル書込みを失敗させるため、この一段前の失敗を検査しない。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| A | 解決記録の親フォルダを事前条件とし、存在しなければIssue更新前に拒否する | 最も単純 | 差は無視できる | 差は無視できる | 新しいフォルダを正規に作る現在の用途を狭める | コードと試験 | 小 | 一変更を戻せる |
| B | 親フォルダ作成をIssue更新前へ移し、作成失敗を安定した不合格へ変換する | 単純 | 差は無視できる | 差は無視できる | Issue更新前に失敗するため片残りを防ぎ、新規pathも維持 | コードと試験 | 小 | 一変更を戻せる |
| C | 親フォルダ作成を現在の位置に残し、解決記録作成全体を復元処理で囲む | やや複雑 | 差は無視できる | 差は無視できる | 復元できるが、変更前に避けられる失敗後の復元へ依存 | コードと試験 | Bより大きい | 一変更を戻せる |

【判断】修正を行うなら案Bが最小かつ頑健である。ただし、これは現役化や状態反映とは別のコード修正作業であり、
Human承認、失敗を再現する試験、実装修正、独立完了レビューを要する。
