...
計数はsession記録とGit履歴から再構成した（時刻の自動計測は未導入）。

| §10計測項目 | 実測 | 備考 |
| --- | --- | --- |
| 着手指示から範囲固定までの時間 | 同一session内（2026-08-09） | mode宣言→SCOPE v1 commitまで途中停止なし |
| Humanによる受け渡し回数 | 約11回 | scope往復4回、完了レビュー往復6回、Closer関連1回 |
| Humanによる承認・裁定回数 | 5回 | mode宣言（risk確定含む）、Closer確定、分割案1裁定、RED再開承認、Finding修正承認×2（RR-P1-001〜003、RR-P1-004） |
| 実装前の範囲修正件数 | 1回（scope v1→v2） | 範囲レビューv1のFinding 4件（P1×2、P2×2）と分割裁定を反映 |
| 範囲レビューの周回 | 2周＋再評価record 1件 | 再評価はReviewer自身による過剰判定の訂正（blocking→0、条件2件） |
...
| `reported_unverified`／`report_execution_mismatch`の有無 | あり（範囲1回、完了2回）→最終`verified` | いずれもblocking Findingの列挙を伴う |
