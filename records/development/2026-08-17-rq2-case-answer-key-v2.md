# RQ2 paired trial ケース正解表 v2（確定・材料path改訂）
...
- 状態：**確定**（利用者文言「正解表v1を確定」2026-08-17 chat）。実起動後の正解の事後変更は
  行わない（変更が必要になったら理由をrecordして別版を立てる）
- **v2改訂の理由**：契約は対象pathが`docs/`配下であることを要求するため（事前走査
  `records/development/2026-08-17-rq2-apparatus-prescan-v1.md` §0-1）、材料を
  `tools/evaluation/fixtures/rq2/`から`docs/evaluation/rq2-cases/`へ移設した。**v1との差分は
  path表記だけ**であり、答・採点基準・限界の記述は1文字も変えていない（`diff`で機械確認済み：
  差分28行はすべてpath行）。file内容は不変のためdigestも不変である
- 材料：`docs/evaluation/rq2-cases/case-001/`〜`case-010/`

## 1. 材料のdigest表【機械出力の転記】

`find docs/evaluation/rq2-cases -name "*.md" | sort | xargs shasum -a 256` の出力そのまま。
...
50437d998848421bb7ae85eafc0f4c11b8ae519ea0190dbfae6488e1a58ec483  docs/evaluation/rq2-cases/case-002/prescan-digest-record.md
850dfcbe6aff70f1f9f54deb75bd4377001284e697e8bd0cd06bf6230e95ca95  docs/evaluation/rq2-cases/case-003/contract-interpretation-scope.md
b600cb2ffe48186c01b650fa41187f206f19c817f1171fad1a565dc89dd4f2ad  docs/evaluation/rq2-cases/case-003/procedure-result-reading.md
...
57145f3824b64632f536c30414be0b359f275007b964bf2b603a1f3ce61bd693  docs/evaluation/rq2-cases/case-005/reviewer-launch-e2e-evidence.md
e675f3aeb1a1e753f535fb9de7465de94e81c8933359acab0eeabd6f64d092eb  docs/evaluation/rq2-cases/case-006/reviewer-bridge-work-ticket.md
...
4a0ac0ce0835a811cc0657c2f2ce850b3808afd6a4c9e0b3fa252a970caae961  docs/evaluation/rq2-cases/case-009/product-acceptance-decision.md
a422e4e01a3ed7d17d107078bdd6755f25fd2071b36a759c59de79ddf5dfd223  docs/evaluation/rq2-cases/case-010/launch-metrics-work-ticket.md
```
...
## 6. 実験計画v1への是正提案（提起した3点の状態）

| # | 対象 | 現状（計画v1） | 状態 |
| --- | --- | --- | --- |
| 1 | 測定量 | §2「依頼recordの材料選択・prompt bytesが不変」 | **裁定済み（2026-08-17）**：起動方式ごとに定める。CLI方式は材料選択の不変性＋reviewer入力トークン、API方式はprompt bytesのまま。`records/development/2026-08-17-rq2-measurement-and-pool-decision-v1.md` §1 |
| 2 | 正解表の配置 | §4「確定recordをcommitしてから実起動」 | **未裁定**。推奨＝事前登録（pre-registration）形：正解の本体はrepo外私有領域へ置き、repoには本表のSHA-256を封じたrecordだけを実起動前にcommitし、実験後に本体をcommitしてdigest照合する。汚染の遮断と事後変更の否定を同時に満たす。実起動（後続3）の前に要る |
| 3 | 条件Cの資料 | §2「他ケース材料＋汎用文書」 | **裁定済み（2026-08-17）**：他ケース材料を使わず、どのケースの期待Findingにも触れない独立プールとする。選定手続きは同decision record §2 |

裁定済み2点は装置実装（後続2）の前提を満たした。**本表の確定（計画§4-2）とは別の判断**である。
