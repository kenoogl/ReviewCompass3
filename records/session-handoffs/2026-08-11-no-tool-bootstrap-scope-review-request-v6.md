...
- supersedes：`records/session-handoffs/2026-08-11-no-tool-bootstrap-scope-review-request-v5.md`
...
| SR5所見Human裁定 | `24b9ce2c3da2d839a7eaf647b09ffc79bb192c5a` | `records/session-handoffs/2026-08-11-no-tool-bootstrap-scope-review-prompt-sr5-human-decision-v1.md` | `68a7b740dbde5091242214f80421eec342ee5c430f0cb0e4ddd87c098184ede5` |
...
### 2.1 範囲固定v3内部の固定入力と不一致時の停止

§1の対象commitにある範囲固定v3のGit blobから、§3「固定入力」の表を機械抽出する。表は
`identity`、`path`、`SHA-256`の3列、データ12行でなければならず、identityとpathの重複を認めない。
抽出不能、列・行数の違い、重複、空値、64桁小文字16進数でないSHA-256が一件でもあれば、レビュー課題へ
進まず、`verdict: reported_unverified`、`stop_reason: stale_input`で停止する。

抽出した12件の各行について、次を全件機械照合する。

1. pathがrepository内の既存の通常fileであり、symbolic linkではない。
2. 対象commitにある同pathのGit blobのSHA-256が、表中SHA-256と一致する。
3. 現在fileのbytesのSHA-256が、表中SHA-256と一致する。

一件でも欠落または不一致なら、`SR-CB-002`以降へ進まず、`verdict: reported_unverified`、
`stop_reason: stale_input`とし、未評価の課題と不一致のidentity、pathだけを列挙して停止する。秘密値や
file内容は結果へ出さない。

§1、§2、§2.1の全件が一致した場合だけ、独立レビュー課題へ進む。§2.1の12件を§2へ重複転記せず、
対象commitの範囲固定v3 §3を固定された一覧の正本とする。

## 3. 独立レビュー課題

- `SR-CB-001`：対象、固定材料、範囲固定v3内部の固定入力12件について、§1、§2、§2.1の方法で機械照合する。
...
- `AC-SR-CB-001`：§1、§2、§2.1の全入力が一致した場合だけ`SR-CB-002`以降へ進み、不一致時は
  `reported_unverified`／`stale_input`で止まる。
...
- `ST-SR-CB-001`：§1、§2、§2.1の一件でも不一致なら`reported_unverified`／`stale_input`で停止し、
  `SR-CB-002`以降を実行しない。
...
- `OUT-SR-CB-002`：§1、§2、§2.1で実行した機械確認と、安全な反証を、単独commandごとの終了code・
  要約とともに示す。
