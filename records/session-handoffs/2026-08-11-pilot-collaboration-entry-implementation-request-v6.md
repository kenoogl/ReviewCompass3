...
- Human安全裁定：`PA-PC-008とPA-PC-009を全件採用し、修正した単一v5指示書を作る`
- Human限定裁定：`PA-PC-010を採用し、入力内容とSHA-256の計算対象を統一した単一v6指示書を作る`
- 開始時基準コミット：`4dcb0bc5c964b537eac0332794a3f84aec90f037`
- 対象コミット：本依頼を含むコミットを起動時に機械取得して固定する
- 旧依頼：`records/session-handoffs/2026-08-11-pilot-collaboration-entry-implementation-request-v5.md`
- Human限定裁定記録：`records/session-handoffs/2026-08-11-pilot-collaboration-pa-pc-010-human-decision-v1.md`
...
本書は旧依頼v5を置き換える、自己完結した単一の実装依頼である。実装担当はv5以前を指示として併用せず、
本書だけを実装指示の正本とする。旧依頼、品質確認記録、Human裁定は改訂理由の履歴としてだけ参照する。
開始設定の`instruction.path`と`instruction.sha256`は本書一件だけへ束縛する。

本改訂は、入力payload bytesとそのSHA-256計算対象の矛盾を解消する限定修正である。要求26件、段階数、
変更可能範囲、実装範囲をv5から増やさない。
...
監査・判定の両envelopeで、`prompt_payload_bytes`を`envelope_sha256` keyを除いたenvelope objectの正規JSON
bytesと定義する。これは末尾改行を含まない。実起動接続後に監査または判定へ渡す入力は、この
`prompt_payload_bytes`そのものだけとする。`envelope_sha256`は`prompt_payload_bytes`のSHA-256である。
保存するenvelope file全体は、`envelope_sha256`を含む正規JSON bytesと末尾改行であり、入力payloadとして
送らず、入力同一性のSHA-256計算対象にも使わない。

これは発見力モードであり、参照範囲を閉包したとは主張しない。

### `AC-PC-004` ingest入力、起動記録、未加工結果の不変保存

`ingest`は、絶対pathで指定されたUTF-8 raw fileと起動記録fileを読む。raw fileと起動記録は通常fileだけを
受け付け、symlinkを拒否する。完全形式、run ID、stage、attempt ID、model、raw SHA-256、commandとstatusの
整合を検査した後、意味上の解析や入力一致・起動成否の判定より先に両方を不変保存する。

起動記録は次の11 keyを持つ完全形式objectとする。
...
| `input_payload_sha256` | 実際に呼出しへ渡した`prompt_payload_bytes`のsha256 |
...
`status: succeeded`は全commandの終了コードが0の場合だけ、`status: failed`は1件以上のcommandが0以外の場合
だけ受け付ける。不整合は`launch_record_invalid`として不変保存前に停止する。起動不能を表す場合も、呼出しを
試みたargvと観測済み終了コードを持てないため、本縦切りのingest対象にはせず、`launch_record_invalid`で
停止する。実起動不能の表現は実起動接続の後続作業へ移管する。
...
保存するlaunch documentは起動記録へ`launch_record_sha256`を追加した12 keyとし、追加keyを除く正規JSON
SHA-256を値とする。raw documentは`schema_version`、`run_id`、`stage`、`attempt_id`、
`input_payload_sha256`、`launch_record_sha256`、`raw_sha256`、`raw_text`の8 keyを持つ。raw documentの
`input_payload_sha256`は起動記録の同値をそのまま保存し、現在のenvelope値へ置き換えない。

両fileを保存して再読込した後、次の順で判定する。

1. 起動記録の`input_payload_sha256`を、stageが`prompt_audit`なら`prompt-audit-envelope.json`、
   `prompt_judgment`なら`prompt-judgment-envelope.json`から再計算した`prompt_payload_bytes`のSHA-256と比較する。
   この再計算値は検証済み`envelope_sha256`とも一致しなければならない。不一致なら解析せず、対応する
   `input_mismatch` eventを追記し、`input_payload_mismatch`で`blocked`へ停止する。
2. 入力が一致し、起動記録の`status`が`failed`なら解析せず、対応する`launch_failed` eventを追記し、
   `launch_failed`で`blocked`へ停止する。
3. 入力が一致し、`status`が`succeeded`の場合だけ、段階に対応するrawの意味上の解析を開始する。

入力不一致と起動失敗が同時なら入力不一致を先に採用する。どの停止でも保存済みlaunch、raw、eventを
上書きしない。同一run内でattempt IDを再利用せず、保存済みfile、parsed file、eventを上書きしない。
...
指示文監査rawは、対応する起動記録が`status: succeeded`で、記録された`input_payload_sha256`が現在の
監査`prompt_payload_bytes`のSHA-256と一致する場合だけ解析する。次の5 keyを持つJSON一文書だけを受け付ける。
...
`pilot-prompt-judgment-v1`とする。`envelope_sha256`は§3 `AC-PC-003`で定義した判定用`prompt_payload_bytes`の
SHA-256とする。

### `AC-PC-006` 指示文判定結果の完全形式と厳格解析

指示文判定rawは、対応する起動記録が`status: succeeded`で、記録された`input_payload_sha256`が現在の
判定`prompt_payload_bytes`のSHA-256と一致する場合だけ解析する。次の5 keyを持つJSON一文書だけを受け付ける。
...
| `input_sha256` | 入力configまたは対応する現在の`prompt_payload_bytes`のSHA-256 |
| `output_sha256` | 成功時はmanifestまたは解析結果SHA-256、失敗時はraw SHA-256 |
| `status` | `completed`または`failed` |
| `stop_code` | 成功時は`null`、失敗時は§3 `AC-PC-008`の対応code |

event file名、event type、合法な遷移、導出状態を次に固定する。表の`入力一致`は、起動記録の
`input_payload_sha256`と対応する現在の`prompt_payload_bytes`のSHA-256が同値であることを指す。

| file | event type | 前段 | 条件 | 導出状態 |
| --- | --- | --- | --- | --- |
| `0001-prepared.json` | `prepared` | なし | prepare成功 | `ready_for_prompt_audit` |
| `0002-prompt-audit-input-mismatch.json` | `prompt_audit_input_mismatch` | prepared | raw・launch保存後、入力不一致 | `blocked` |
| `0002-prompt-audit-launch-failed.json` | `prompt_audit_launch_failed` | prepared | raw・launch保存後、入力一致・起動失敗 | `blocked` |
| `0002-prompt-audit-parsed.json` | `prompt_audit_parsed` | prepared | 入力一致・起動成功・監査解析成功 | `ready_for_prompt_judgment` |
| `0002-prompt-audit-parse-failed.json` | `prompt_audit_parse_failed` | prepared | 入力一致・起動成功・raw保存後に監査解析失敗 | `blocked` |
| `0003-prompt-judgment-input-mismatch.json` | `prompt_judgment_input_mismatch` | audit parsed | raw・launch保存後、入力不一致 | `blocked` |
| `0003-prompt-judgment-launch-failed.json` | `prompt_judgment_launch_failed` | audit parsed | raw・launch保存後、入力一致・起動失敗 | `blocked` |
| `0003-prompt-judgment-parsed.json` | `prompt_judgment_parsed` | audit parsed | 入力一致・起動成功・判定解析成功・findingあり | `human_decision_required` |
| 同上 | 同上 | audit parsed | 入力一致・起動成功・判定解析成功・findingなし | `ready_for_executor` |
| `0003-prompt-judgment-parse-failed.json` | `prompt_judgment_parse_failed` | audit parsed | 入力一致・起動成功・raw保存後に判定解析失敗 | `blocked` |

失敗eventは`status: failed`、`output_sha256`は保存済みraw bytesのSHA-256とする。`input_mismatch` eventの
`stop_code`は`input_payload_mismatch`、`launch_failed` eventの`stop_code`は`launch_failed`、解析失敗eventは
対応する解析停止codeを持つ。

`status`はmanifest、全event、launch、raw、parsed、固定commit blob、現在fileを再読込し、正規JSON、SHA-256、
前段、入力、出力、attempt IDを再照合して状態を導出する。各launchとrawに保存した
`input_payload_sha256`同士の一致、launchの`status`、command終了コード、event type、stop codeも再照合する。
parsed、launch failed、parse failedのeventでは保存値と現在の`prompt_payload_bytes`のSHA-256が一致すること、
input mismatchのeventでは一致しないことを確認する。各envelopeの`envelope_sha256`も同じ
`prompt_payload_bytes`から再計算する。固定commit blobと現在fileが異なる場合はeventを書き換えず`stale`を返す。
未知event、番号欠落、合法表外の遷移、保存物改竄は`blocked`とする。
...
input_payload_mismatch, launch_failed, raw_digest_mismatch, raw_parse_failed,
coverage_incomplete, audit_digest_mismatch, transition_invalid, stored_record_invalid,
stale_input
```

`input_payload_mismatch`と`launch_failed`はraw・launch・失敗eventの不変保存後に返す。他の保存前検査停止では
eventを追加しない。未知引数、必須引数欠落、引数型違いは`config_invalid`または`stage_invalid`へ正規化して
終了コード2とする。標準エラーへ判定結果を分散させない。
...
- `NG-PC-003` 未加工結果を解析前に捨てない。入力不一致、起動失敗、解析失敗を成功へ変換しない。
...
- `OUT-PC-003` 正常例、負例、境界例に加え、SHA-256改竄、未加工結果上書き、要求被覆欠落、所見全件照合欠落、
  段階飛ばし、private root誤配置、起動失敗、古い入力payload SHA-256、起動状態と終了コードの不一致、
  保存envelope file全体のSHA-256を誤って入力照合へ使う実装を故障注入で検出する。
