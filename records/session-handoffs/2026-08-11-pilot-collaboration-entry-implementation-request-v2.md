...
- 旧依頼：`records/session-handoffs/2026-08-11-pilot-collaboration-entry-implementation-request-v1.md`
- Human裁定：`records/session-handoffs/2026-08-11-pilot-collaboration-entry-prompt-findings-human-decision-v1.md`
...
本書は旧依頼v1を置き換える自己完結した実装依頼である。実装担当はv1を指示として併用せず、本書だけを
実装指示の正本とする。v1と品質確認記録は改訂理由の履歴としてだけ参照する。
...
`instruction`は`path`と`sha256`の2 keyを持つ。`materials`の各要素も`path`と`sha256`の2 keyを持ち、
pathを重複させない。pathはrepository root基準の安全なPOSIX相対pathで、空、絶対path、`.`、`..`、NULを
拒否する。
