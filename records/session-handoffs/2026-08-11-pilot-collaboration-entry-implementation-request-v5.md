...
- 旧依頼：`records/session-handoffs/2026-08-11-pilot-collaboration-entry-implementation-request-v4.md`
- Human安全裁定記録：`records/session-handoffs/2026-08-11-pilot-collaboration-pa-pc-008-009-human-decision-v1.md`
...
本書は旧依頼v4を置き換える、自己完結した単一の実装依頼である。実装担当はv4以前を指示として併用せず、
...
| `input_envelope_sha256` | 実際に呼出しへ渡した段階別envelope bytesのsha256 |
...
`input_envelope_sha256`、`launch_record_sha256`、`raw_sha256`、`raw_text`の8 keyを持つ。raw documentの
`input_envelope_sha256`は起動記録の同値をそのまま保存し、現在のenvelope値へ置き換えない。

両fileを保存して再読込した後、次の順で判定する。

1. 起動記録の`input_envelope_sha256`を、stageが`prompt_audit`なら`prompt-audit-envelope.json`、
   `prompt_judgment`なら`prompt-judgment-envelope.json`の`envelope_sha256`と比較する。不一致なら解析せず、
   対応する`input_mismatch` eventを追記し、`input_envelope_mismatch`で`blocked`へ停止する。
...
指示文監査rawは、対応する起動記録が`status: succeeded`で、記録された`input_envelope_sha256`が現在の
監査envelopeと一致する場合だけ解析する。次の5 keyを持つJSON一文書だけを受け付ける。
...
指示文判定rawは、対応する起動記録が`status: succeeded`で、記録された`input_envelope_sha256`が現在の
判定envelopeと一致する場合だけ解析する。次の5 keyを持つJSON一文書だけを受け付ける。
...
| `input_sha256` | 入力configまたは対応する現在のenvelope SHA-256 |
...
`input_envelope_sha256`と対応する現在のenvelope SHA-256が同値であることを指す。
...
`stop_code`は`input_envelope_mismatch`、`launch_failed` eventの`stop_code`は`launch_failed`、解析失敗eventは
対応する解析停止codeを持つ。
...
`input_envelope_sha256`同士の一致、launchの`status`、command終了コード、event type、stop codeも再照合する。
parsed、launch failed、parse failedのeventでは保存値と現在のenvelopeが一致すること、input mismatchのeventでは
保存値と現在のenvelopeが一致しないことを確認する。固定commit blobと現在fileが異なる場合はeventを書き換えず
`stale`を返す。未知event、番号欠落、合法表外の遷移、保存物改竄は`blocked`とする。
...
input_envelope_mismatch, launch_failed, raw_digest_mismatch, raw_parse_failed,
coverage_incomplete, audit_digest_mismatch, transition_invalid, stored_record_invalid,
stale_input
```

`input_envelope_mismatch`と`launch_failed`はraw・launch・失敗eventの不変保存後に返す。他の保存前検査停止では
...
- `OUT-PC-003` 正常例、負例、境界例に加え、SHA-256改竄、未加工結果上書き、要求被覆欠落、所見全件照合欠落、
  段階飛ばし、private root誤配置、起動失敗、古い入力封筒SHA-256、起動状態と終了コードの不一致を故障注入で
  検出する。
