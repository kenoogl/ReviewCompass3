# 実験ケースcase-001（条件D）の妥当性レビュー 独立確認依頼record（headless起動対象・Claude→Reviewer）
...
reviewcompass3-reviewer-launch launch \
  ...
  --request records/session-handoffs/2026-08-17-rq2-case-001-d-request-v1.md \
  --expected-sha256 <本record commit後のSHA-256> \
  --private-root <repo外私有領域の絶対パス> \
  --run-id <実行識別子>
```

3. アダプタが判定recordを`records/session-handoffs/2026-08-17-rq2-case-001-d-verdict-v1.md`へ機械転記して単独commitし、事後照合4点を実行する。
