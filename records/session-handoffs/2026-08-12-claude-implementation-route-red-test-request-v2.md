# Claude実装委譲経路 第1縦切り RED受入試験作成依頼 v2
...
- 製品実装：禁止
- Claude起動、外部送信：禁止
- 置換元：`2026-08-12-claude-implementation-route-red-test-request-v1.md`
...
- RED開始・所見裁定：`records/session-handoffs/2026-08-12-claude-implementation-route-red-findings-human-decision-v1.md`
...
- 前周監査・判定：`records/session-handoffs/2026-08-12-claude-implementation-route-red-request-audit-raw-v1.json`、
  `records/session-handoffs/2026-08-12-claude-implementation-route-red-request-audit-judgment-raw-v1.json`
...
一つの合成Git repositoryと偽のClaude応答を使い、次の公開入口を期待する受入試験を作る。

- module：`tools.development.claude_implementation_route`
- `prepare(repository, config_path, private_root)`：固定入力、Human承認、能力設定を検査し、repository外の
  一時worktreeと第1ターンの固定起動情報を作る。
- `record_turn(repository, private_root, run_id, turn, launch_path, raw_path)`：未加工結果と道具使用を不変保存し、
  全変更pathを機械照合する。`test`ターンでは機械処理だけが固定試験commandを実行してREDを確認する。
  `implementation`ターンでは固定済み試験指紋を確認して同じcommandのGREENを確認する。
- `status(repository, private_root, run_id)`：保存物を再検査し、状態を導出する。

実際のClaude、認証、管理者配置、外部networkを使わない。Claude processの代わりに、起動記録、未加工応答、
道具使用一覧、変更済み合成worktreeをfixtureとして渡す。Gitと固定試験commandだけは製品側の機械処理が
配列引数で実行する想定にする。
...
4. 実行前照合：Claude Codeの版、実行file指紋、`claude.ai`かつ`firstParty`の認証、Human指定モデルと
   許容応答モデルのいずれかが不一致なら、worktreeと起動記録を作らず停止する。API key由来の認証も拒否する。
5. 切替・再試行：別モデル、別認証、別経路、別送信先への自動切替設定と、失敗時の自動再試行設定を
   `prepare`で拒否し、外部process数が0のままであることを確認する。
6. 変更境界：試験ターンは指定試験pathだけ、実装ターンはproduction pathだけを許す。symlink、未追跡の
   余剰file、主作業ツリー変更、試験指紋変更を拒否する。
7. 管理者配置境界：管理者配置を変更対象に含む設定または書込を示す道具使用記録を拒否し、管理者配置、
   主作業ツリー、起動記録に成果物を作らない。
8. 機械試験：固定command以外、文字列shell、Claude報告だけのRED／GREEN判定を拒否する。終了コードが
   第1ターンで非0、第2ターンで0の場合だけ進む。
9. 保存：起動記録、未加工応答、道具使用、試験結果、変更一覧を上書きせず保存し、欠落・余剰・改竄を
   `status`で拒否する。
10. 後続境界：ClaudeへGit操作をさせず、機械commit前後を区別し、独立レビューとHuman段完了承認なしに
    完了状態へ進まない。
11. 確認運転境界：ReviewCompass3の実repositoryをproof対象にする設定、秘密・利用者情報を含む材料、
    無工具疎通承認の流用を開始前に拒否する。
12. 要求対応：25要求すべてを少なくとも1試験へ対応付け、対応先の試験関数が実在することを機械確認する。
