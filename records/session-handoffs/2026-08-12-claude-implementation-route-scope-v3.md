...
- 開始境界裁定：`2026-08-12-claude-implementation-route-scope-finding-human-decision-v1.md`
- 権限裁定：`2026-08-12-claude-implementation-route-permission-finding-human-decision-v1.md`
- 置換元：`2026-08-12-claude-implementation-route-scope-v2.md`
...
route_implementation_start_approval: human_bound_required
claude_command_tool: disabled
test_executor: reviewcompass_machine
mechanical_assurance_status: specified_only
```

基準コミット、指示書、固定資料、要求集合、結果形式、能力設定、機械処理だけが実行する試験command、
変更可能path、一回限り承認は、作業開始設定でSHA-256へ束縛する。
...
Claudeへ見せる道具は`Read`、`Glob`、`Grep`、`Edit`、`Write`だけとする。`Bash`、Web、MCP、別agentなど、
command実行または外部通信ができる道具は見せない。権限方式は`dontAsk`とし、worktree内の読取と
変更可能pathの編集だけを事前許可する。変更可能path外の編集は自動拒否する。

固定試験commandはClaudeへ渡さず、各ターン終了後にReviewCompass3の機械処理が配列引数で一度だけ実行する。
Claude CodeのBash用OS隔離を合否根拠にしない。物理的な一時worktree、Claude Codeの読取・編集規則、
ターン後の全差分と指紋の機械照合を重ね、主作業ツリーとworktree外への変更を拒否する。

MCPは空設定に固定し、plugin、hook、skill、Chrome、Web、別agent、背景agentを無効にする。外部通信は
AnthropicのHuman承認済みClaude応答経路だけとし、道具によるnetwork通信を許可しない。

## 5. 実行順序

経路の失敗する受入試験作成と製品実装は、対象範囲、変更可能path、承認記録SHA-256へ束縛した別Human承認を
確認した後だけ開始する。承認がない、対象が違う、またはSHA-256が一致しない場合は、file変更前に停止する。
...
4. 同じClaude sessionの第1ターンでは受入試験だけを変更可能にする。ターン終了後に機械処理が固定試験
   commandを実行し、対象試験の不合格を確認する。
5. 第1ターン後の試験file指紋を固定する。第2ターンではproductionの変更可能pathだけを編集可能にし、
   試験fileの変更を拒否する。ターン終了後に機械処理が同じ固定試験commandを実行する。
...
- `AC-CD-001` コミット済み指示書、固定資料、要求集合、能力設定、実装開始Human承認を含む全承認の指紋が
  一致する場合だけClaude実装入口を利用できる。
...
- `AC-CD-004` 同一sessionの試験作成ターンと実装ターンを分け、機械処理によるRED確認後の試験指紋を固定し、
  実装ターンの試験変更を拒否する。
- `AC-CD-005` Claudeは許可した読取・検索・編集道具と編集pathだけを使い、試験commandは機械処理だけが
  完全一致の配列引数で実行し、Claudeへcommand・network道具を与えない。
...
- `NG-CD-002` Claudeへ`Bash`、任意command、隔離外実行、外部networkを許可しない。`bypassPermissions`と
  `auto`を使わない。
...
- `ST-CD-001` 指示文品質、要求被覆、Human裁定、実装開始Human承認、能力設定、その他の承認、固定入力の
  いずれかが不一致である。
- `ST-CD-002` Claude Codeの版、実行file指紋、認証、要求モデルまたは許容応答モデルが一致しない。
- `ST-CD-003` `Bash`、command、network、禁止道具、禁止pathの利用が要求または試行された。
- `ST-CD-004` 第1ターン後の機械試験が期待したREDにならない、または第2ターンで試験fileが変わった。
...
- `OUT-CD-003` 機械処理が実行したRED結果、固定した試験指紋、GREEN結果、関連試験、秘密情報検査の
  command配列と終了コード。
...
Claudeが受入試験を先に作り、その後に純粋関数を実装する。固定試験commandは機械処理が各ターン後に実行する。
Codex主担当は変更せず、機械検証後のcommitを`gpt-5.6-terra`サブエージェントへレビュー依頼する。

確認運転のClaude起動、固定文、合成repository内容、モデル、道具、期限、一回限り承認は、実装完了レビュー後に
別のHuman承認へ束縛する。本範囲固定の承認を実送信承認として扱わない。
